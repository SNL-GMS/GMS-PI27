...
{{- define "gms.common.secrets.copy" }}
  {{- if .Values.copySecrets }}
    {{- range $key, $secretDef := .Values.copySecrets }}
      ...
      {{- end }}
        {{- include "gms.common.secret.copy" (dict "context" $ "namespace" $secretDef.namespace "source_name" ($secretDef.sourceName | default $key) "dest_name" ($secretDef.destName | default $key) "optional" ($secretDef.optional | default false)) }}
    {{- end }}
  {{- end }}
{{- end }}
...
optional: If false, error if secret does not exist. If true, then just continue without creating secret. Default false.
