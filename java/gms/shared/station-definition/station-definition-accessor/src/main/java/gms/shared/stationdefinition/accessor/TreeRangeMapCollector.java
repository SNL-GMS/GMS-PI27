...
public class TreeRangeMapCollector<K extends Comparable<? super K>, V>
    implements Collector<Map.Entry<Range<K>, V>, TreeRangeMap<K, V>, TreeRangeMap<K, V>> {
  ...
  public BinaryOperator<TreeRangeMap<K, V>> combiner() {
    return (TreeRangeMap<K, V> left, TreeRangeMap<K, V> right) -> {
      left.putAll(right);
      return left;
    };
  }
