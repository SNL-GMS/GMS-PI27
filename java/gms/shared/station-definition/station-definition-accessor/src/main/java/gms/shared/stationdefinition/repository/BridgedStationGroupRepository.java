...
/**
 * A {@link StationGroupRepository} implementation that uses a bridged database to provide {@link
 * StationGroup} instances
 */
@Component("bridgedStationGroupRepository")
public class BridgedStationGroupRepository implements StationGroupRepository {
  ...
  public BridgedStationGroupRepository(
      BridgedStationRepository bridgedStationRepository,
  ...
  public List<StationGroup> findStationGroupsByNameAndTime(
    ...
    List<String> networkNames = networkDaos.stream().map(NetworkDao::getNet).toList();

    List<AffiliationDao> affiliationDaos =
        affiliationDatabaseConnector.findAffiliationsByNameAndTime(networkNames, effectiveTime);

    List<AffiliationDao> nextAffiliationDaos =
        affiliationDatabaseConnector.findNextAffiliationByNameAfterTime(
            networkNames, effectiveTime);

    List<String> referenceStations =
        affiliationDaos.stream()
    ...
    List<Station> stations =
        bridgedStationRepository.findStationsByNameAndTime(referenceStations, effectiveTime);

    return stationGroupAssembler.buildAllForTime(
        effectiveTime, networkDaos, affiliationDaos, nextAffiliationDaos, stations);
  }
  ...
  public List<StationGroup> findStationGroupsByNameAndTimeRange(
    ...
    List<AffiliationDao> affiliationDaos =
        affiliationDatabaseConnector.findAffiliationsByNameAndTimeRange(
            networkNames, startTime, endTime);
    ...
    List<AffiliationDao> nextAffiliationDaos =
        affiliationDatabaseConnector.findNextAffiliationByNameAfterTime(networkNames, endTime);
    List<Station> stations =
        bridgedStationRepository.findStationsByNameAndTimeRange(
            referenceStations, startTime, endTime);

    return stationGroupAssembler.buildAllForTimeRange(
        Range.closed(startTime, endTime),
        networkDaos,
        affiliationDaos,
        nextAffiliationDaos,
        stations);
