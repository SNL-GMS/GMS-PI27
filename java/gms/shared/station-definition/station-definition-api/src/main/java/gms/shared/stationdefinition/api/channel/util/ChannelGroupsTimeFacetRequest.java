...
/**
 * A {@link Request} for faceted {@link ChannelGroup}s by time from the station definition endpoint
 */
@AutoValue
...
public abstract class ChannelGroupsTimeFacetRequest implements Request {
  ...
  public interface Builder {
    ...
    ChannelGroupsTimeFacetRequest.Builder setChannelGroupNames(
        ImmutableList<String> channelGroupNames);

    ChannelGroupsTimeFacetRequest.Builder setEffectiveTime(Optional<Instant> effectiveTime);

    ChannelGroupsTimeFacetRequest.Builder setFacetingDefinition(
        Optional<FacetingDefinition> facetingDefinition);

    default ChannelGroupsTimeFacetRequest.Builder setChannelGroupNames(
        Collection<String> channelGroupNames) {
    ...
    default ChannelGroupsTimeFacetRequest.Builder setFacetingDefinition(
    ...
    default ChannelGroupsTimeFacetRequest build() {
      var channelGroupsTimeFacetRequest = autoBuild();
      Validate.notEmpty(
          channelGroupsTimeFacetRequest.getChannelGroupNames(),
          "Channel groups time facet request must be provided a list of channel group names");
      channelGroupsTimeFacetRequest
          .getEffectiveTime()
          .ifPresent(
              data ->
                  Preconditions.checkState(
                      channelGroupsTimeFacetRequest.getEffectiveTime().isPresent()));
      channelGroupsTimeFacetRequest
          .getFacetingDefinition()
          .ifPresent(
              data ->
                  Preconditions.checkState(
                      channelGroupsTimeFacetRequest.getFacetingDefinition().isPresent()));
