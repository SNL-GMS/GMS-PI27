...
public class StationDefinitionManager {
  ...
  private static final long TEN_MINUTES_IN_MILIS = 600_000;
  private static final long HOUR_IN_MILIS = 3_600_000;
  ...
  public StationDefinitionManager(
      StationDefinitionAccessor stationDefinitionAccessor, CacheAccessor cacheAccessor) {
    this.stationDefinitionAccessor = stationDefinitionAccessor;
    this.cacheAccessor = cacheAccessor;
  }

  // Schedule cron for loading the cache on time interval
  @Scheduled(initialDelay = TEN_MINUTES_IN_MILIS, fixedRate = HOUR_IN_MILIS)
   ...
   * optional effective time instant and optional faceting definition. If the list is empty, the
   * server will return an empty list.
   *
   * @param request The collections of station groups names, effective time of the groups and
   *     faceting definition
  ...
  @Operation(
      summary =
          "retrieves all station groups specified by a list of station group names, "
              + "effectiveTime and facetingDefinition")
  public List<StationGroup> findStationGroupsByName(
      @io.swagger.v3.oas.annotations.parameters.RequestBody(
              description =
                  "List of station group names, " + "effective time and faceting definition")
          @RequestBody
          StationGroupsTimeFacetRequest request) {

    List<String> stationGroupNames = request.getStationGroupNames();
    Instant effectiveTime = request.getEffectiveTime().orElseGet(Instant::now);

    if (request.getFacetingDefinition().isPresent()) {
      return stationDefinitionAccessor.findStationGroupsByNameAndTime(
          stationGroupNames,
          effectiveTime,
          request
              .getFacetingDefinition()
              .orElseThrow(() -> new IllegalArgumentException(FACETING_ERROR_MESSAGE)));
    } else {
      return stationDefinitionAccessor.findStationGroupsByNameAndTime(
          stationGroupNames, effectiveTime);
    }
  }

  /**
   * Retrieves all {@link StationGroup} objects specified by a list of station group names and time
   * range bounding the groups. If the list is empty, the server will return an empty list.
   *
  ...
  @Operation(
      summary =
          "retrieves all station groups specified by a list of station group names and time range")
  public List<StationGroup> findStationGroupsByNameAndTimeRange(
      @io.swagger.v3.oas.annotations.parameters.RequestBody(
              description = "List of station group names and time range")
          @RequestBody
          StationGroupsTimeRangeRequest request) {

    return stationDefinitionAccessor.findStationGroupsByNameAndTimeRange(
        request.getStationGroupNames(),
        request.getTimeRange().getStartTime(),
        request.getTimeRange().getEndTime());
  }

  /**
   * Returns a list of {@link Station} objects that corresponds to the list of {@link
   * StationsTimeRangeRequest#getStationNames()} passed in with optional effective time and optional
   * faceting definition. If an empty list is provided, this method will return an empty list of
   * stations.
   *
   * @param request {@link StationsTimeFacetRequest} facet request object
   * @return list of all {@link Station} objects for the given set of station names that have an
   *     {@link Station#getEffectiveAt()} that is with in the specified time range of the request.
   */
  ...
  public List<Station> findStationsByName(
      @io.swagger.v3.oas.annotations.parameters.RequestBody(
              description = "List of station names, effective time," + " and faceting definition")
          @RequestBody
    ...
    Instant effectiveTime = request.getEffectiveTime().orElseGet(Instant::now);
    if (request.getFacetingDefinition().isPresent()) {
      return stationDefinitionAccessor.findStationsByNameAndTime(
          stationNames,
          effectiveTime,
          request
              .getFacetingDefinition()
  ...
  /**
   * Retrieves all {@link Station} objects specified by a list of station names and time range
   * bounding the station. If the list is empty, the server will return an empty list of stations.
   *
  ...
  public List<Station> findStationsByNameAndTimeRange(
      @io.swagger.v3.oas.annotations.parameters.RequestBody(
              description = "List of station names and time range")
          @RequestBody
          StationsTimeRangeRequest request) {

    return stationDefinitionAccessor.findStationsByNameAndTimeRange(
        request.getStationNames(),
        request.getTimeRange().getStartTime(),
        request.getTimeRange().getEndTime());
  }

  /**
   * Retrieves the times at which the provided {@link Station} and any of its aggregate objects
   * changed, within the provided time range.
   *
   * @param request the {@link Station} and bounding time range
   * @return a collection of {@link Instant}s representing times when new versions of the {@link
   *     Station} and/or any of its aggregate objects became effective
   */
  ...
  public List<Instant> determineStationChangeTimes(
      @io.swagger.v3.oas.annotations.parameters.RequestBody(
              description = "Name of station and start and end time range")
          @RequestBody
          StationChangeTimesRequest request) {

    return stationDefinitionAccessor.determineStationChangeTimes(
        request.getStation(), request.getStartTime(), request.getEndTime());
  }

  /**
   * Retrieves all {@link ChannelGroup} objects specified by a list of channel group names, with
   * optional effective time and optional faceting definition. If the list is empty, the server will
   * return an empty list of channel groups.
   *
  ...
  @Operation(
      summary =
          "retrieves all channel groups specified by a list of channel group names, "
              + "effectiveTime and facetingDefinition")
  public List<ChannelGroup> findChannelGroupsByName(
      @io.swagger.v3.oas.annotations.parameters.RequestBody(
              description =
                  "List of channel group names, effective time," + " and faceting definition")
          @RequestBody
    ...
    if (request.getFacetingDefinition().isPresent()) {
      return stationDefinitionAccessor.findChannelGroupsByNameAndTime(
          channelGroupNames,
          effectiveTime,
          request
              .getFacetingDefinition()
  ...
  /**
   * Retrieves all {@link ChannelGroup} objects specified by a list of channel group names and time
   * range bounding the groups. If the list is empty, the server will return an empty list of
   * channel groups.
   *
  ...
  public List<ChannelGroup> findChannelGroupsByNameAndTimeRange(
      @io.swagger.v3.oas.annotations.parameters.RequestBody(
              description = "List of channel group names and time range")
          @RequestBody
          ChannelGroupsTimeRangeRequest request) {

    return stationDefinitionAccessor.findChannelGroupsByNameAndTimeRange(
  ...
  /**
   * Retrieves all {@link Channel} objects specified by a list of channel names, with optional
   * effective time and optional faceting definition. If the list is empty, the server will return
  ...
  @Operation(
      summary =
          "retrieves all channels specified by a list of channel names, and optionally, an"
              + "effectiveTime and facetingDefinition")
  public List<Channel> findChannels(
      @io.swagger.v3.oas.annotations.parameters.RequestBody(
              description = "List of channel names, effective time," + " and faceting definition")
          @RequestBody
          ChannelsTimeFacetRequest request) {

    Instant time = request.getEffectiveTime().orElseGet(Instant::now);
  ...
  /**
   * Retrieves all {@link Channel} objects specified by a list of channel names and time range
   * bounding the groups. If the list is empty, the server will return an empty list of channels.
   *
  ...
  public List<Channel> findChannelsByNameAndTimeRange(
      @io.swagger.v3.oas.annotations.parameters.RequestBody(
              description = "List of channel names and time range")
          @RequestBody
          ChannelsTimeRangeRequest request) {

    return stationDefinitionAccessor.findChannelsByNameAndTimeRange(
        request.getChannelNames(),
        request.getTimeRange().getStartTime(),
        request.getTimeRange().getEndTime());
  }
  ...
  /**
   * Returns a list of {@link Response} objects that corresponds to the list of Response Ids passed
   * in.
   *
   * <p>If no ids are passed into this interface, an empty list of Responses is returned. if no time
   * is provided to the optional effectiveTime, the time of request is the default if no faceting
   * definition is provided, FAP is faceted by ID
   *
  ...
  public List<Response> findResponsesById(
      @io.swagger.v3.oas.annotations.parameters.RequestBody(
              description = "List of response ids, effective time," + " and faceting definition")
          @RequestBody
          ResponseTimeFacetRequest request) {

    return request.getFacetingDefinition().isPresent()
        ? stationDefinitionAccessor.findResponsesById(
            request.getResponseIds(),
            request.getEffectiveTime().orElseGet(Instant::now),
            request
                .getFacetingDefinition()
                .orElseThrow(() -> new IllegalArgumentException(FACETING_ERROR_MESSAGE)))
        : stationDefinitionAccessor.findResponsesById(
            request.getResponseIds(), request.getEffectiveTime().orElseGet(Instant::now));
  }
   ...
   *
   * <p>If no ids are passed into the interface, an empty list of Responses is returned.
   *
  ...
  public List<Response> findResponsesByIdAndTimeRange(
      @io.swagger.v3.oas.annotations.parameters.RequestBody(
              description = "List of response ids and time range")
          @RequestBody
          ResponseTimeRangeRequest request) {

    return stationDefinitionAccessor.findResponsesByIdAndTimeRange(
        request.getResponseIds(),
        request.getTimeRange().getStartTime(),
        request.getTimeRange().getEndTime());
  }
