...
public final class DefaultCoiTestFixtures {
  ...
  public static SiteChanDao getDefaultSiteChanDaoFromSiteDao(SiteDao siteDao) {
    ...
    var defaultSiteChanKey =
        new SiteChanKey(siteDao.getId().getStationCode(), CHAN_1, siteDao.getId().getOnDate());
    var siteChanDao = new SiteChanDao();
    siteChanDao.setId(defaultSiteChanKey);
    siteChanDao.setChannelId(DEFAULT_CHANID);
  ...
  public static Channel getDefaultChannelFromSiteAndSiteChan(
      SiteDao siteDao, SiteChanDao siteChanDao) {
  ...
  public static ChannelGroup getDefaultChannelGroupFromSiteAndChannel(
      SiteDao siteDao, Channel channel) {
  ...
  public static ChannelGroup getDefaultChannelGroupNoStation(SiteDao siteDao, Channel channel) {
  ...
  public static ChannelGroup getChannelGroup(
      String name, List<Channel> channels, Instant effectiveAt) {
    ...
    return ChannelGroup.builder().setName(name).setEffectiveAt(effectiveAt).setData(data).build();
  }

  public static Station getDefaultStationFromChannelGroup(
      ChannelGroup channelGroup, SiteDao siteDao) {

    List<RelativePositionChannelPair> pairs =
        channelGroup.getChannels().stream()
            .map(
                channel ->
                    RelativePositionChannelPair.create(
                        RelativePosition.from(DEFAULT_DOUBLE, DEFAULT_DOUBLE, DEFAULT_DOUBLE),
                        channel))
  ...
  public static Station getDefaultStation() {
    ...
    return Station.builder().setName(DEFAULT_STRING).setEffectiveAt(START).build();
  }
  ...
  public static Station getDefaultStation(String name, Instant effectiveAt) {
    ...
    List<RelativePositionChannelPair> pairs =
        List.of(
            RelativePositionChannelPair.create(
                RelativePosition.from(DEFAULT_DOUBLE, DEFAULT_DOUBLE, DEFAULT_DOUBLE), channel));
    ...
    var data =
        Station.Data.builder()
            ...
            .setEffectiveUntil(Instant.MAX)
            .setChannelGroups(
                List.of(getDefaultChannelGroupFromSiteAndChannel(getDefaultSiteDao(), channel)))
            .setAllRawChannels(List.of(channel))
            .build();

    return Station.builder().setName(name).setEffectiveAt(effectiveAt).setData(data).build();
  }

  public static Station getDefaultStationForTime(String name, Instant effectiveAt) {

    var channel = getDefaultChannel().toEntityReference();
    List<RelativePositionChannelPair> pairs =
        List.of(
            RelativePositionChannelPair.create(
                RelativePosition.from(DEFAULT_DOUBLE, DEFAULT_DOUBLE, DEFAULT_DOUBLE), channel));
    ...
    var data =
        Station.Data.builder()
            ...
            .setEffectiveUntil(Instant.MAX)
            .setChannelGroups(
                List.of(getDefaultChannelGroupNoStation(getDefaultSiteDao(), channel)))
            .setAllRawChannels(List.of(channel))
            .build();
  ...
  public static Station getDefaultStationForTime(
      String stationString,
      String channelGroupString,
      List<String> channelStrings,
      Instant effectiveAt) {

    List<Channel> channels =
        channelStrings.stream()
            .map(
                chanString ->
                    stationString
                        + DEFAULT_SEPARATOR
                        ...
                        + DEFAULT_SEPARATOR
                        + chanString)
            .map(chanName -> Channel.createEntityReference(chanName))
            .toList();
    ...
    var data =
        Station.Data.builder()
            ...
            .setLocation(getDefaultLocation())
            .setEffectiveUntil(Instant.MAX)
            .setChannelGroups(List.of(getChannelGroup(channelGroupString, channels, effectiveAt)))
            .setAllRawChannels(channels)
            .build();

    return Station.builder()
        .setName(stationString)
  ...
  public static Location getDefaultLocation() {
    ...
    return Location.from(DEFAULT_DOUBLE, DEFAULT_DOUBLE, DEFAULT_DOUBLE, DEFAULT_DOUBLE);
  }

  public static Channel getDefaultChannel() {

    return getDefaultChannel(DEFAULT_STRING);
  }

  public static Channel getDefaultChannel(String name) {
    ...
    var data =
        Channel.Data.builder()
            ...
            .setProcessingMetadata(
                Map.of(ChannelProcessingMetadataType.CHANNEL_GROUP, DEFAULT_STRING))
            .setResponse(Optional.empty())
            .setEffectiveUntil(END)
            .build();

    return Channel.builder().setName(name).setEffectiveAt(START).setData(data).build();
  }

  public static AffiliationDao getDefaultAffiliationDao(
      String net, String sta, Instant time, Instant endTime) {
