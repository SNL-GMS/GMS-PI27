...
public abstract class StationGroup implements Comparable<StationGroup> {
  ...
  public interface Builder {
    ...
    default StationGroup build() {
      ...
      stationGroup
          .getData()
          .ifPresent(data -> Preconditions.checkState(stationGroup.getEffectiveAt().isPresent()));
      return autoBuild();
    }
  }

  public static StationGroup createEntityReference(String name) {
    return StationGroup.builder().setName(name).build();
  }

  public StationGroup toEntityReference() {
    return new AutoValue_StationGroup.Builder().setName(getName()).build();
  }

  public static StationGroup createVersionReference(String name, Instant effectiveAt) {
    Objects.requireNonNull(effectiveAt);
    return StationGroup.builder().setName(name).setEffectiveAt(effectiveAt).build();
  }
  ...
  public abstract static class Data {
    ...
    public interface Builder {
      ...
      default StationGroup.Data build() {
        ...
        final List<Optional<?>> allFields = List.of(getDescription(), getStations());
        ...
        if (0 == numPresentFields) {
          ...
          Validate.notEmpty(
              getDescription().orElseThrow(), "Station group must be provided a description");
          Validate.notEmpty(
              getStations().orElseThrow(), "Station group must have a non-empty list of stations");
