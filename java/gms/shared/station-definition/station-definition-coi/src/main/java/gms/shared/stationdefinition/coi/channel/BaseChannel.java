...
/**
 * Base class for {@link Channel} that also serves as the Channel's DTO. The primary difference from
 * Channel is this class has a getStationId operation that is used in serialization but does not
 * have a getStation operation (i.e. it only has a reference to Station). The effect of this is a
 * serialized Channel can only ever have a reference to it's parent Station.
 *
 * <p>Could update Channel constructors to force the Station to be a reference; could log warnings
 * during serialization if the Station is not a reference, etc. to let users know of this
 * serialization limitation.
 *
 * <p>Ideally this class would be package-private or protected, but that causes problems for the
 * current ChannelComparator implementation.
 */
abstract class BaseChannel {
  ...
  public abstract Optional<? extends BaseChannel.Data> getData();
