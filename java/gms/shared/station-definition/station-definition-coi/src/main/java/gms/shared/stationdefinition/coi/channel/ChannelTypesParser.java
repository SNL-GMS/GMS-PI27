...
public final class ChannelTypesParser {

  private static final Logger LOGGER = LoggerFactory.getLogger(ChannelTypesParser.class);

  private static final Map<Character, ChannelTypes> weatherTypesByCode =
      Map.ofEntries(
          entry(
              'S',
   ...
   * Manages such edge cases as two-character versus three-character channels, and the weather
   * channels.
   *
   * @param chan two or three character combination of channel codes from a sitechan record (e.g.
   *     SHZ)
  ...
  private static Optional<ChannelTypes> handleThreeCharacterChannel(
      char bandCode, char instrumentCode, char orientationCode) {
    ...
    var chan =
        new StringBuilder()
            .append(bandCode)
            .append(instrumentCode)
            .append(orientationCode)
            .toString();
    if (NON_CONVENTIONAL_INFRASOUND_CHANNELS.contains(chan)) {
      // orientation code is A for these channels, but should be F by convention, so it is set to F
      // here
    ...
    Optional<ChannelDataType> dataTypeOptional =
        ChannelDataTypeConverter.getChannelDataType(instrumentType, corectedOrientationCode);

    if (dataTypeOptional.isEmpty()) {
      LOGGER.warn(
          "No ChannelDataType found for instrument {}, orientation code {}",
          instrumentType,
          corectedOrientationCode);
