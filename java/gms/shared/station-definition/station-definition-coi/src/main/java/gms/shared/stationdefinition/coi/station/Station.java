...
public abstract class Station implements StationDefinitionObject, Comparable<Station> {
  ...
  public interface Builder {
    ...
    default Station build() {
      ...
      station
          .getData()
          .ifPresent(data -> Preconditions.checkState(station.getEffectiveAt().isPresent()));
  ...
  public static Station createEntityReference(String name) {
    return Station.builder().setName(name).build();
  }

  public Station toEntityReference() {
    return new AutoValue_Station.Builder().setName(getName()).build();
  }
  ...
  public static Station createVersionReference(String name, Instant effectiveAt) {
    ...
    return Station.builder().setName(name).setEffectiveAt(effectiveAt).build();
  }

  public static Station createVersionReference(Station other) {
      ...
      default Station.Data build() {
        ...
        final List<Optional<?>> allFields =
            List.of(
                getType(),
                getDescription(),
                getRelativePositionChannelPairs(),
                getLocation(),
                getChannelGroups(),
                getAllRawChannels());
        ...
        if (0 == numPresentFields) {
          ...
          Validate.notEmpty(
              getAllRawChannels().orElseThrow(), "Station must have a non-empty list of channels");
          Validate.isTrue(
              getDescription().orElseThrow().length() <= MAX_DESCRIPTION_LENGTH,
              "Descriptions can be no longer than 1024 characters");
          Validate.notEmpty(
              getChannelGroups().orElseThrow(),
              "Station must have a non-empty list of channel groups");
          Validate.notEmpty(
              getRelativePositionChannelPairs().orElseThrow(),
              "Station being passed an empty or null map of relative positions for channels it"
                  + " manages");
          setRelativePositionChannelPairs(
              getRelativePositionChannelPairs().orElseThrow().stream()
                  .sorted(
                      Comparator.comparing(
                          Functions.compose(
                              Channel::getName, RelativePositionChannelPair::getChannel)))
                  .toList());
          ...
          Map<Channel, RelativePosition> relativePositionsByChannel =
              getRelativePositionChannelPairs().orElseThrow().stream()
                  .collect(
                      Collectors.toMap(
                          pair -> pair.getChannel().toEntityReference(),
          ...
          getAllRawChannels()
              ...
              .forEach(
                  channel ->
                      Validate.isTrue(
                          relativePositionsByChannel.containsKey(channel.toEntityReference()),
                          "Station passed in a relative position for a channel it does not manage "
                              + channel.getName()));

          final var channelNames =
              getAllRawChannels().orElseThrow().stream().map(Channel::getName).toList();

          final var channelGroups = getChannelGroups().orElseThrow();

          Validate.isTrue(
              channelGroups.stream().noneMatch(ChannelGroup::isPresent)
                  || channelNames.stream()
                      .allMatch(
    ...
    public Map<Channel, RelativePosition> getRelativePositionsByChannel() {
      return getRelativePositionChannelPairs().stream()
          .collect(
              Collectors.toMap(
                  Functions.compose(
                      Channel::toEntityReference, RelativePositionChannelPair::getChannel),
                  RelativePositionChannelPair::getRelativePosition));
    }
