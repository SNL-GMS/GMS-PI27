...
public abstract class ChannelGroup implements StationDefinitionObject, Comparable<ChannelGroup> {
  ...
  public interface Builder {
    ...
    default ChannelGroup build() {
      ...
      channelGroup
          .getData()
          .ifPresent(data -> Preconditions.checkState(channelGroup.getEffectiveAt().isPresent()));
      return channelGroup;
    }
  }

  public static ChannelGroup createEntityReference(String name) {
    return new AutoValue_ChannelGroup.Builder().setName(name).build();
  }

  public ChannelGroup toEntityReference() {
    return new AutoValue_ChannelGroup.Builder().setName(getName()).build();
  }

  public static ChannelGroup createEntityReference(ChannelGroup channelGroup) {
    Objects.requireNonNull(channelGroup);
    return new AutoValue_ChannelGroup.Builder().setName(channelGroup.getName()).build();
  }
  ...
  public static ChannelGroup createVersionReference(String name, Instant effectiveAt) {
    ...
    return new AutoValue_ChannelGroup.Builder().setName(name).setEffectiveAt(effectiveAt).build();
  }
    ...
    public interface Builder {
      ...
      default ChannelGroup.Data build() {
        ...
        if (0 == numPresentFields && getLocation() == null) {
          ...
          Validate.notEmpty(
              getChannels().orElseThrow(), "ChannelGroup must have a non-zero number of channels");
