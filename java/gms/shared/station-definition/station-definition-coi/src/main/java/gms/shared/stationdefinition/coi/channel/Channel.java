 ...
 */
@AutoValue
@JsonSerialize(as = BaseChannel.class)
@JsonDeserialize(builder = AutoValue_Channel.Builder.class)
public abstract class Channel extends BaseChannel
    implements StationDefinitionObject, Comparable<Channel> {
  ...
  public static Channel createEntityReference(String name) {
    return new AutoValue_Channel.Builder().setName(name).build();
  }

  public static Channel createEntityReference(Channel channel) {
    Objects.requireNonNull(channel);
    return new AutoValue_Channel.Builder().setName(channel.getName()).build();
  }

  public static Channel createVersionReference(String name, Instant effectiveAt) {
    Objects.requireNonNull(effectiveAt);
    return new AutoValue_Channel.Builder().setName(name).setEffectiveAt(effectiveAt).build();
  }
  ...
  /**
   * Determines if the Channel is derived based on its name
   *
   * @param channel Channel to verify
   * @return True if a derived channel, false otherwise
   */
  public static boolean isDerivedChannel(Channel channel) {
    Objects.requireNonNull(channel);

    // TODO: (Future) Add Fk and Event beam checks when those are implemented
    return channel.getName().contains("/filter")
        || channel.getName().contains("/masked")
        || channel.getName().contains("/beam");
  }
                ...
                .orElseThrow(
                    () ->
                        new IllegalStateException(
                            "Cannot create version id from entity instantiation")))
        .build();
  }

  public Channel toEntityReference() {
    return new AutoValue_Channel.Builder().setName(getName()).build();
  }

  public StationDefinitionObject setEffectiveAt(Instant effectiveAt) {
    return this.toBuilder().setEffectiveAt(effectiveAt).build();
  }

  public StationDefinitionObject setEffectiveUntil(Instant effectiveUntil) {
    return this.toBuilder()
        .setData(this.getData().orElseThrow().toBuilder().setEffectiveUntil(effectiveUntil).build())
        .build();
  }
  ...
  public Optional<Orientation> getOrientationAngles() {
          ...
          .ifPresent(
              data ->
                  Preconditions.checkState(
                      channel.getEffectiveAt().isPresent(),
  ...
  public abstract static class Data extends BaseChannel.Data {
    ...
    List<ChannelId> getChannelId() {
      return getConfiguredInputs().stream()
          .map(
              (Channel ci) -> {
    ...
    public interface Builder {
      ...
      private static boolean orientationCodesMatch(
        ...
        if (ChannelOrientationType.UNKNOWN != orientationType) {
          return Character.toUpperCase(orientationType.getCode())
              == Character.toUpperCase(orientationCode);
        }
      ...
      default Channel.Data build() {
        ...
        final List<Optional<?>> allFields =
            List.of(
                ...
                getChannelDataType(),
                getChannelBandType(),
                getChannelInstrumentType(),
                getChannelOrientationType(),
                getChannelOrientationCode(),
                getUnits(),
                getNominalSampleRateHz(),
                getLocation(),
                getOrientationAngles(),
                getConfiguredInputs(),
                getProcessingDefinition(),
        ...
        if (0 == numPresentFields) {
          ...
          Validate.notEmpty(
              getCanonicalName().orElseThrow(), "Channel must be provided a canonical name");
          ...
          getConfiguredInputs()
              ...
              .forEach(
                  ci ->
                      Validate.notNull(
                          ci, "None of the configured inputs for a channel should be null"));
          ...
          Validate.isTrue(
              !Character.isWhitespace(getChannelOrientationCode().orElseThrow()),
              "Channel's channelOrientationCode cannot be whitespace");

          Validate.isTrue(
              orientationCodesMatch(
                  getChannelOrientationType().orElseThrow(),
