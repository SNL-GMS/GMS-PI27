...
final class ChannelDataTypeConverter {
  ...
  private static final Set<ChannelInstrumentType> orientedWeatherInstruments =
      Set.of(TEMPERATURE, PRESSURE, HUMIDITY);

  private static final Map<Character, ChannelDataType> weatherOrientationToDataType =
      Map.ofEntries(
          entry('A', WEATHER),
          entry('O', WEATHER),
   ...
   */
  static Optional<ChannelDataType> getChannelDataType(
      ChannelInstrumentType instrumentType, char orientationCode) {
  ...
  private static Optional<ChannelDataType> getWeatherDataTypeForOrientationCode(
