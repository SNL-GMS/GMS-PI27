...
public final class StationDefinitionVersionUtility {
  ...
  public static List<WfdiscDao> getWfDiscsAsSingleVersion(Collection<WfdiscDao> wfdiscDaos) {
    return StationDefinitionVersionUtility.getWfDiscsWithVersionEndTime(wfdiscDaos)
        .values()
        .stream()
        .map(
            (Pair<WfdiscDao, WfdiscDao> wfdiscPair) -> {
  ...
  public static List<WfdiscDao> getWfDiscsWithNextVersion(Collection<WfdiscDao> wfdiscDaos) {
    return StationDefinitionVersionUtility.getWfDiscsWithVersionEndTime(wfdiscDaos)
        ...
        .map(
            (Pair<WfdiscDao, WfdiscDao> wfdiscPair) -> {
              var newDao = new WfdiscDao(wfdiscPair.getLeft());
              if (wfdiscPair.getLeft().getVersionAttributeHash()
                  == wfdiscPair.getRight().getVersionAttributeHash()) {
                newDao.setEndTime(wfdiscPair.getRight().getEndTime());
              } else {
  ...
  /**
   * Group wfdisc into correct sta.chan bins and return the first wfDisc in the list with it's
   * effectiveUntil set to the last wfdisc endTime.
   *
   * <p>This will be used in setting response versions
   *
  ...
  private static LinkedHashMap<String, Pair<WfdiscDao, WfdiscDao>> getWfDiscsWithVersionEndTime(
    ...
    // bin wfDisc into sta.chan with a Pair of first/last wfdisc in list (will be used to create a
    // version)
    return wfdiscDaos.stream()
        .collect(
            groupingBy(
                wfdisc ->
                    createStationChannelCode(wfdisc.getStationCode(), wfdisc.getChannelCode()),
                LinkedHashMap::new,
  ...
  /**
   * Group sensor into correct sta.chan bins and return the first sensor in the list with it's
   * effectiveUntil set to the last sensor endTime.
   *
   * <p>This will be used in setting response versions
   *
  ...
  public static List<SensorDao> getSensorsWithVersionEndTime(Collection<SensorDao> sensorDaos) {
    Objects.requireNonNull(sensorDaos);

    // bin sensor into sta.chan with a Pair of first/last wfdisc in list (will be used to create a
    // version)
    LinkedHashMap<String, Pair<SensorDao, SensorDao>> sensorTimes =
        sensorDaos.stream()
            .collect(
                groupingBy(
                    sensor ->
                        createStationChannelCode(
                    ...
                    Collectors.collectingAndThen(
                        ...
                        list ->
                            Pair.of(
                                Collections.min(
                                    list, Comparator.comparing(x -> x.getSensorKey().getTime())),
                                Collections.max(
                                    list,
                                    Comparator.comparing(x -> x.getSensorKey().getEndTime()))))));

    // set EndTime of Version
    return sensorTimes.values().stream()
        .map(
            (Pair<SensorDao, SensorDao> x) -> {
              var dao = x.getLeft();
   ...
   * @param versionAttributeHashFunction function to compute hash for elements that create a
   *     version. used to determine when there is a new version
   * @param versionTimeHashFunction function that returns time-sta-chan hash to act as unique key
   *     for version Map
   * @param <T> elements to bin by version
   * @return
   */
  public static <T, U> NavigableMap<U, List<T>> getVersionMapAsInt(
      Collection<T> nonVersionsList,
      ToIntFunction<T> versionAttributeHashFunction,
    ...
    for (T version : nonVersionsList) {
      ...
      if (firstVersion == null
          || versionAttributeHashFunction.applyAsInt(firstVersion)
              != versionAttributeHashFunction.applyAsInt(version)) {
