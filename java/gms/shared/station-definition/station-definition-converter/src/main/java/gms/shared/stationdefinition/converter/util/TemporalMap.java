 ...
 */
public final class TemporalMap<K, V> {
  ...
  public static <K, V> TemporalMap<K, V> create() {
    return new TemporalMap<>(new HashMap<>());
  }
  ...
  public void putAll(TemporalMap<K, V> map) {
    map.keySet().forEach(key -> getVersionMap(key).putAll(map.getVersionMap(key)));
  }
  ...
  public Optional<V> getVersionFloor(K key, Instant time) {
    return Optional.ofNullable(getVersionMap(key).floorEntry(time)).map(Entry::getValue);
  }
  ...
  public Optional<V> getVersionCeiling(K key, Instant time) {
    return Optional.ofNullable(getVersionMap(key).ceilingEntry(time)).map(Entry::getValue);
  }
  ...
  /**
   * Factory method for generating a Stream Collector to build a TemporalMap from a Stream of values
   *
   ...
   */
  public static class TemporalMapCollector<K, V>
      implements Collector<V, TemporalMap<K, V>, TemporalMap<K, V>> {
    ...
    public BinaryOperator<TemporalMap<K, V>> combiner() {
      return (TemporalMap<K, V> left, TemporalMap<K, V> right) -> {
        left.putAll(right);
        return left;
      };
    }
