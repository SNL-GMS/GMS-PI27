...
/**
 * Converts both Interactive and Automatic Processing {@link StageInterval}s to and from legacy
 * {@link IntervalDao}s
 */
@Component
public class IntervalConverter {

  private static final String STAGE_MATCH_ERROR_MSG = "No matching stage found for %s intervals";
  ...
  public IntervalConverter(Workflow workflow) {
    ...
    this.stagesByName =
        workflow.getStages().stream().collect(Collectors.toMap(Stage::getName, identity()));
  }

  /**
   * Attempts to convert a legacy {@link IntervalDao} into a COI {@link StageInterval} using known
   * conversions from the legacy stages to configured system {@link Stage}s. The kind of
   * StageInterval generated is dependent on the kind of Stage the IntervalDao comes from.
   *
  ...
  public StageInterval convert(IntervalDao intervalDao) {
    var stage =
        IntervalUtility.getStageName(intervalDao.getType(), intervalDao.getName())
            .flatMap(this::getStage)
            .orElseThrow(
                () ->
                    new IllegalArgumentException(
                        format(
                            "No matching stage found for interval %s:%s",
                            intervalDao.getType(), intervalDao.getName())));

    return convertInternal(intervalDao, stage);
  }

  /**
   * Batch conversion method for converting legacy {@link IntervalDao}s from a wide variety of
   * stages. Note that the query times used to retrieve the IntervalDaos is included, as certain
   * stages will require procedurally generated intervals that do not always map one-to-one with a
   * legacy IntervalDao. These stages will use a converter where the expected interval time ranges
   * are provided. Ordering will be respected as best as possible in the return based on the input
   * ordering of the IntervalDaos, given the grouping by Stage that occurs.
   *
   ...
   */
  public Map<String, List<StageInterval>> convert(
      List<IntervalDao> intervalDaos, Instant queryStart, Instant queryEnd) {
            ...
            Collectors.groupingBy(
                intervalDao ->
                    IntervalUtility.getStageName(intervalDao.getType(), intervalDao.getName())
                        .flatMap(this::getStage)))
        .forEach(
            (Optional<Stage> stageOpt, List<IntervalDao> intervalGroup) -> {
              var stage =
                  stageOpt.orElseThrow(
                      () ->
                          new IllegalArgumentException(
                              STAGE_MATCH_ERROR_MSG.formatted(intervalGroup.size())));
              addStageToMap(stage, intervalGroup, intervalsByStageName, queryStart, queryEnd);
            });
  ...
  private void addStageToMap(
      Stage stage,
      List<IntervalDao> intervalGroup,
      Map<String, List<StageInterval>> intervalsByStageName,
      Instant queryStart,
      Instant queryEnd) {
    if (IntervalUtility.AUTO_NETWORK.equals(stage.getName())) {
      intervalsByStageName.put(
          stage.getName(),
          convertForAutoNetwork(
              intervalGroup, (AutomaticProcessingStage) stage, queryStart, queryEnd));
    } else {
      intervalsByStageName.put(
          stage.getName(),
          intervalGroup.stream().map(interval -> convertInternal(interval, stage)).toList());
    }
  }

  private StageInterval convertInternal(IntervalDao intervalDao, Stage stage) {
    return StageMode.INTERACTIVE == stage.getMode()
  ...
  private List<StageInterval> convertForAutoNetwork(
      List<IntervalDao> autoIntervalDaos,
    ...
    for (Instant currentStart = shiftedStart;
        currentStart.isBefore(shiftedEnd);
        currentStart = currentStart.plus(stage.getDuration())) {
      currentEnd = currentStart.plus(stage.getDuration());
      latestForTime =
          autoIntervalDaos.stream()
              .filter(encloses(currentStart, currentEnd))
              .max(
                  Comparator.comparing(IntervalDao::getLastModificationDate)
                      .thenComparing(IntervalDao::getLoadDate));

      if (latestForTime.isPresent()) {
        stageIntervals.add(
            automaticIntervalConverter.fromLegacy(
                latestForTime.get(), stage, currentStart, currentEnd));
      }
    }
  ...
  private static Instant findBestStartTime(List<IntervalDao> intervalDaos, Instant queryStartTime) {
    var minIntervalTime = intervalDaos.stream().mapToDouble(IntervalDao::getTime).min();

    var minIntervalStart =
        minIntervalTime.isPresent()
  ...
  private static Instant findBestEndTime(List<IntervalDao> intervalDaos, Instant queryEndTime) {
    var maxIntervalEndTime = intervalDaos.stream().mapToDouble(IntervalDao::getEndTime).max();

    var maxIntervalEnd =
        maxIntervalEndTime.isPresent()
  ...
  private static Instant getAdjustedStartTime(Instant startTime, Duration stageDuration) {
    var startEpochSeconds = startTime.getEpochSecond();
    long stageDurationSeconds = stageDuration.getSeconds();
    checkArgument(
        stageDurationSeconds > 0, "Error adjusting start time, stage duration must be > 0");
  ...
  private static Instant getAdjustedEndTime(Instant endTime, Duration stageDuration) {
  ...
  /**
   * Predicate generator for filtering out {@link IntervalDao}s that do not enclose the input time
   * range. Used to narrow down the possible IntervalDaos we'll use to generate a Auto Network
   * {@link StageInterval}
   *
   * @param intervalStart Start of generated interval
   * @param intervalEnd End of generated interval
   * @return A Predicate that returns true if the IntervalDao encloses the generated interval, false
   *     otherwise
   */
