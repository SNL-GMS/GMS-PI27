...
public abstract class TravelTimeLookupTable implements TravelTimeDepthDistanceLookupTablePlugin {
  ...
  private final String errorMessageUninitializedPlugin =
      String.format("Method called on uninitialized plugin, %s.", this.getClass());
  ...
  protected TravelTimeLookupTable(
      FileStore fileStore, TravelTimeLookupTableConfiguration configuration) {
  ...
  public void initialize() {
    ...
    LOGGER.info(
        "Initializing data for bucket/key {}, table class name {}",
  ...
  private void validatePhaseType(PhaseType phaseType) {
    if (!viewMap.containsKey(phaseType)) {
      throw new IllegalArgumentException(
          String.format("Invalid PhaseType arg, %s, in plugin, %s.", phaseType, this.getClass()));
    }
  }
