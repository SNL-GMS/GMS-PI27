...
/**
 * Utility class for performing Bicubic spline interpolation, as well as related (loosely or
 * otherwise) operations such as partial derivatives.
 */
public class ApacheBicubicSplineInterpolator {
  ...
  public ApacheBicubicSplineInterpolator() {
    ...
    univariateFunctionDifferentiator =
        new FiniteDifferencesDifferentiator(
            NUMBER_OF_FINITE_DIFFERENCE_POINTS, FINITE_DIFFERENCE_STEP_SIZE);
  }
   ...
   * @return a BivariateFunction that, given (x, y), interpolates on the table and returns an
   *     interpolated value.
   */
  BivariateFunction interpolate(double[] xValues, double[] yValues, double[][] functionValues) {

    validateInput(xValues, yValues, functionValues);

    var interpolator = new PiecewiseBicubicSplineInterpolator();

    return interpolator.interpolate(xValues, yValues, functionValues);
  }
  ...
  public double differentiateTravelTime(
      FeaturePredictionDerivativeType derivativeType,
      BivariateFunction function,
      double distance,
      double depth) {

    if (derivativeType == FeaturePredictionDerivativeType.DERIVATIVE_WRT_DISTANCE) {
      return travelTimeWrtDistanceDerivative(function, distance, depth);
    } else {
      throw new NotImplementedException(
          "Derivative for " + derivativeType + " not yet implemented");
    }
  }
  ...
  private double travelTimeWrtDistanceDerivative(
    ...
    return differentiatedFunction
        .value(new DerivativeStructure(1, 1, 0, distance))
        .getPartialDerivative(1);
  }

  private static void validateInput(double[] xValues, double[] yValues, double[][] functionValues) {

    Validate.notNull(xValues, "xValues is null");
    Validate.isTrue(
        xValues.length >= MIN_GRID_SIZE, "not enough xValues, need at least " + MIN_GRID_SIZE);

    Validate.notNull(yValues, "yValues is null");
    Validate.isTrue(
        yValues.length >= MIN_GRID_SIZE, "not enough yValues, need at least " + MIN_GRID_SIZE);

    Validate.notNull(functionValues, "functionValues is null");

    Validate.isTrue(
        xValues.length == functionValues.length,
        "Dimension mismatch: the length of xValues (%d) does not match the first dimension of"
            + " functionValues (%d)",
        xValues.length,
    ...
    Validate.isTrue(
        ...
        "Dimension mismatch: the length of yValues (%d) does not match the second dimension of"
            + " functionValues (%d)",
        yValues.length,
        functionValues[0].length);
  }
}
