...
public class CustomBicubicSplineInterpolator {
  ...
  private static class Bracket {
     ...
     * monotonically increasing vector (axis). On exit the values a, b, and h are assigned which are
     * used to perform the actual value, derivative, and 2nd derivative interpolation.
     *
  ...
  public BiFunction<Double, Double, double[]> getFunctionAndDerivatives(
    ...
    BiFunction<Double, Double, double[]> transposeInterpolator =
        (yintrp, xintrp) ->
            interpolate(
                yGrid,
                xGrid,
                transposeMatrix(values),
                new Bracket(yintrp, y),
                new Bracket(xintrp, x));

    return (Double xintrp, Double yintrp) -> {
      double[] derivativeSet1 =
      ...
      // double crossDerivative = interpolatedCrossDerivative(transposeInterpolator, yintrp,
      // xintrp);
   ...
   *
   * <p>Based On the function "splin2": Press, W.H. et al., 1988, "Numerical Recipes", 94-110.
   *
  ...
  private static double[] interpolate(
      double[] xAxis, double[] yAxis, double[][] values, Bracket xBracket, Bracket yBracket) {
    // calculate the 2nd derivatives at each of the tables values for splines
    // running in the y directions (secondDerivativeY)

    var secondDerivativeY = new double[xAxis.length][yAxis.length];
    for (var j = 0; j < xAxis.length; j++) {
      naturalSpline(yAxis, values[j], secondDerivativeY[j]);
    }
    // spline(yAxis, values[j], 1.0e30, 1.0e30, secondDerivativeY[j]);
    ...
    for (var j = 0; j < xAxis.length; j++) {
      valuesOnXAtYinterp[j] = interpolatedValue(yBracket, values[j], secondDerivativeY[j]);
    }

    // calculate xgrid second derivative entries along the y interpolation
    // location and save in secondDerivativeX

    var secondDerivativeX = new double[xAxis.length];
    naturalSpline(xAxis, valuesOnXAtYinterp, secondDerivativeX);

    // spline(xAxis, valuesOnXAtYinterp, 1.0e30, 1.0e30, secondDerivativeX);

    // save the interpolate value, derivative, and 2nd derivative

    return new double[] {
      interpolatedValue(xBracket, valuesOnXAtYinterp, secondDerivativeX),
      interpolatedDerivative(xBracket, valuesOnXAtYinterp, secondDerivativeX),
      interpolated2ndDerivative(xBracket, secondDerivativeX)
    };
  }
   ...
   *
   * <p>Function. Given arrays x[0..n-1] and y[0..n-1] containing a tabulated function, i.e., y[i] =
   ...
   * points x[i].
   *
   * <p>If yp1 and/or ypn are equal to 1.0e30 or larger, the routine is signaled to set the
   * corresponding boundary condition for a natural spline, with zero second derivative on that
   * boundary.
   *
   * <p>NOTE: This routine only needs to be called once to process the entire tabulated function in
   * x and y arrays.
   *
   * <p>Based On the function "spline": Press, W.H. et al., 1988, "Numerical Recipes", 94-110.
   *
  ...
  private static double interpolatedValue(Bracket bracket, double[] values, double[] d2v) {
    return bracket.a * values[bracket.klo]
        + bracket.b * values[bracket.khi]
  ...
  private static double interpolatedDerivative(Bracket bracket, double[] values, double[] d2v) {
    return ((values[bracket.khi] - values[bracket.klo]) / bracket.h)
        - (((3.0 * bracket.a * bracket.a - 1.0) * bracket.h * d2v[bracket.klo]) / 6.0)
        + (((3.0 * bracket.b * bracket.b - 1.0) * bracket.h * d2v[bracket.khi]) / 6.0);
  }
  ...
  private static double interpolatedCrossDerivative(
      double[] xGrid, double[] yGrid, double[][] values, double x, double y) {

    final var EPSILON = 1e-7;

    double upperXderivative =
        interpolate(xGrid, yGrid, values, new Bracket(x, xGrid), new Bracket(y + EPSILON, yGrid))[
            1];
    double lowerXderivative =
        interpolate(xGrid, yGrid, values, new Bracket(x, xGrid), new Bracket(y - EPSILON, yGrid))[
            1];
