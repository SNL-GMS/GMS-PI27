...
public class EarthModelUtility {
  ...
  private static final double STARTING_LOWER_BOUND = 181.0;
  ...
  public double[] interpolateEarthModel(double firstModelValue, double secondModelValue) {
    ...
    return bicubicSplineInterpolator
        .getFunctionAndDerivatives(
            miniTableTriple.getLeft(), miniTableTriple.getMiddle(), miniTableTriple.getRight())
   ...
   */
  private Triple<double[], double[], double[][]> generateMinitable(double depth, double distance) {
    ...
    var ttcsInHoleDist = new double[] {STARTING_LOWER_BOUND, -1., 0.};
    ...
    for (int k = 0, kk = zTop; k < nzReq; k++, kk++) {
      ...
      if (inHole || iDist > 0) {
        ...
        for (var j = 0; j < nxReq; j++) {
          miniTable[k][j] =
              checkForInvalidMiniTableEntry(
                  miniTable[k][j], miniDistEx, minTableRowEx, miniDist[j]);
        }
  ...
  private void reconstructMiniTable(
      double[][] miniTable, double[] depths, double depth, double[] miniDepth) {
    ...
    for (var j = 0; j < nxReq; j++) {
      ...
      if (iDepth > 0) {
        ...
        // Extrapolate a new set of depths bracketing the requested depth
        ...
        // We're good if iDepth == 0.0; take no action
      }
    }
  }

  private void shiftDistancesToRequested(
      double[] distances, double distance, double[] miniDist, double[][] miniTable, int k) {
    ...
    for (var j = 0; j < nxReq; j++) {
      miniTable[k][j] =
          checkForInvalidMiniTableEntry(miniTable[k][j], miniDist, minTableRow, miniDist[j]);
    }
  }
  ...
  private boolean determineDepthRange(double[] depths, double depth) {
    if (depths.length == 1) {
      ...
      zBottom = 0;
      // TODO: tolerance?
      if (!DoubleMath.fuzzyEquals(depth, depths[0], DEPTH_PROXIMITY_TOLERANCE)) {
        // TODO: handle this
        return false;
      }
    } else {
  ...
  private void validRangeVectorCalc(
      double[][] miniTable, double[] distances, double[] miniDist, double[] valueRow, int k) {
    // Scan distances (j) in the mini
    for (var j = 0; j < nxReq; j++) {
  ...
  private void getValidSamplesAdvanceDistance(
      double[] valueRow, double[] miniTableRow, double[] distances, double[] miniDist) {
    var numExtrap = 0;
    var numSamp = 0;
    int i = xLow;
    ...
    for (var n = 0; n < numExtrap; n++) {
      miniTableRow[n] =
          checkForInvalidMiniTableEntryExtracted(
              miniTableRow[n], distances, valueRow, numSamp, miniDist[n], i);
    }
  }

  private int countSamples(double[] valueRow) {
    var numSamp = 0;
  ...
  private void getValidSamplesForDepth(
      double[] valueRow,
      double[] miniTableRow,
      double[] distances,
      double[] miniDist,
    ...
    for (var n = 0; n < numExtrap; n++) {
      ...
      miniTableRow[m] =
          checkForInvalidMiniTableEntryExtracted(
              miniTableRow[m], distances, valueRow, numSamp, miniDist[m], i);
    }
  }
  ...
  private void makeFinalAdjustmenDistanceRange(double[][] values, boolean inHole) {
    if (inHole) {
      ...
      // We're good if iDepth == 0; take no action
    }
  }
  ...
  private void calculateDepthRange(
      double depth, double[] depths, double[] miniDepth, int index, int j) {
    wasExtrapolated = true;
  ...
  private void extrapolateRequestedDepth(
      double[][] miniTable, double[] depths, double[][] miniTableTrans, int j, double[] miniDepth) {
    for (var i = 0; i < nzReq; i++) {
      miniTable[i][j] =
          RationalFunctionInterpolator.interpolate(
              extract(depths, zTop, nzReq), extract(miniTableTrans[j], 0, nzReq), miniDepth[i]);
    }
  }
  ...
  private double checkForInvalidMiniTableEntryExtracted(
      double miniTableEntry,
      double[] distances,
      double[] value,
      int numSamp,
      double miniDistEntry,
      int i) {
    if (invalid(miniTableEntry)) {
      wasExtrapolated = true;
      return RationalFunctionInterpolator.interpolate(
          extract(distances, i, numSamp), extract(value, i, numSamp), miniDistEntry);
    }
    return miniTableEntry;
  }

  private double checkForInvalidMiniTableEntry(
      double miniTableEntry, double[] miniDist, double[] minTableRow, double miniDistEntry) {
    if (invalid(miniTableEntry)) {
      wasExtrapolated = true;
  ...
  private static void checkForMinSampleDataException(boolean testThis)
  ...
  private int checkDistValueHighOrHole(
      double[] distances,
      double distance,
      double[] valueArray,
      int index,
  ...
  private static int findIndex(double[] valueArray, double[] distances, int ii, int border) {
  ...
  private void checkDistanceInTableNotHole(double[][] values) {
    ...
    if (valid(values[zTop][0]) && invalid(values[zTop][xHigh])) {
      ...
      // this means the lower distance bound is in a bad sample region
      wasExtrapolated = true;
      ...
      // Both upper and lower bounds are good; take no action
    }
  }
  ...
  private void setDepthRange(double[] depths, double depth) {
    ...
    if (zLeft < 0) {
      ...
      zTop = 0;
      zBottom = nzReq - 1;
    } else if (zLeft >= depths.length - 1) {
      // depth > max. table depth
      iDepth++;
  ...
  private static boolean isInHole(double distance, double[] ttcsInHoleDist) {
    return (distance > ttcsInHoleDist[0] && distance < ttcsInHoleDist[1]);
  }

  /** Something here... */
  private static void setDistInHole(
      double[] ttcsInHoleDist, double[] distances, double[][] values) {
    var okSoFar = true;
    for (var i = 1; i < distances.length; i++) {
      if (!Double.isNaN(values[0][i - 1]) && Double.isNaN(values[0][i])) {
        ...
        // Both values are numbers; take no action
      }
    }
  }
  ...
  private static int hunt(double[] values, double x) {
    if (DoubleMath.fuzzyEquals(x, values[values.length - 1], DISTANCE_PROXIMITY_TOLERANCE)) {
