...
public class BicubicSplineFeaturePredictorConfiguration {
   ...
   */
  @Bean
  public BicubicSplineFeaturePredictorDefinition
      getCurrentBicubicSplineFeaturePredictorDefinition() {

    var configName = "bicubic-spline-feature-predictor";

    boolean extrapolate =
        (boolean) configurationConsumerUtility.resolve(configName, List.of()).get("extrapolate");

    var rawElliptictyCorrectionMap =
        (Map<?, ?>)
            configurationConsumerUtility
                .resolve(configName, List.of())
                .get("ellipticityCorrectorPluginNameByCorrectionType");

    var ellipticityCorrectionsMap =
        rawElliptictyCorrectionMap.entrySet().stream()
            .map(
                (var entry) -> {
                  var key = entry.getKey().toString();
                  return Map.entry(
                      EllipticityCorrectionType.valueOf(key), entry.getValue().toString());
                })
            .collect(Collectors.toMap(Map.Entry::getKey, Map.Entry::getValue));

    var rawTravelTimePluginsMap =
        (Map<?, ?>)
            configurationConsumerUtility
                .resolve(configName, List.of())
                .get("travelTimeLookupTableByEarthModel");

    var travelTimePluginsMap =
        rawTravelTimePluginsMap.entrySet().stream()
    ...
    return BicubicSplineFeaturePredictorDefinition.builder()
        ...
        .setEllipticityCorrectorPluginNameByEllipticityCorrectionPluginType(
            ellipticityCorrectionsMap)
