...
class ControlFactoryWorker {
   ...
   * each {@link Path} annotated operation in the controlClass. The service will be stopped when the
  ...
  <T> T runService(Class<T> controlClass) {
    final var context = createControlContext(controlClass);
  ...
  <T> T createControl(Class<T> controlClass, ControlContext controlContext) {
    ...
    final var creator = findControlCreatorMethod(controlClass);
  ...
  ControlContext createControlContext(Class<?> controlClass) {
    final String controlName =
        AnnotationUtils.findClassAnnotation(controlClass, Component.class)
            .orElseThrow(
                () ->
                    new IllegalArgumentException(
                        "Can only instantiate classes annotated with @Component."))
            .value();
   ...
   *     creator method
   */
  private static <T> T instantiateControl(
      Class<T> controlClass, Method creator, ControlContext controlContext) {
    try {
  ...
  private static Method findControlCreatorMethod(Class<?> controlClass) {
    ...
    final List<Method> factoryMethods =
        Arrays.stream(controlClass.getDeclaredMethods())
