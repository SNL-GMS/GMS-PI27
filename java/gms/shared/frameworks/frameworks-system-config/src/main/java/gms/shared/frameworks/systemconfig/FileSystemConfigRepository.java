 ...
 *
 * <p>If the file specified by filename is not present, this interface will still be valid but will
 * return no configuration values. A warning will be logged for this case.
 */
public final class FileSystemConfigRepository implements SystemConfigRepository {
  ...
  /**
   * Instantiate a FileSystemConfigRepository Note: The filename should include all directory info
   * from the base path on.
   */
  private FileSystemConfigRepository(String filename) {
    ...
    if (StringUtils.isNotEmpty(filePath)) {
      try {
        ...
        LOGGER.warn(
            "Failed to read system configuration file: {}.  Mesage is {}",
            filePath,
            e.getMessage());
      }
    } else {
  ...
  public String toString() {
    return "file:" + Objects.toString(filePath, "none");
  }
