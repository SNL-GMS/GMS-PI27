 ...
 * }</pre>
 *
 * <p>While most components want to share this value, we may want to define a <i>different</i> value
 * specific to the 'spacemodulator' component (perhaps as a temporary override for development or
 * testing). We can do that by prefixing the component name:
 *
 * <pre>{@code
 * spacemodulator.port = 591
 * }</pre>
 *
 * <p>Thus the 'spacemodulator' component can always just call {@code getValueAsInt('port')}. This
 * will first look for the value of port with our component name ('spacemodulator.port') and,
 * failing to find that, will then just return the value of 'port'.
 *
...
public final class SystemConfig {
  ...
  /**
   * Creates a new {@link SystemConfig} for a component with the provided name. Values are gathered
  ...
  /**
   * Creates a new {@link SystemConfig} for a component with the provided name. Values are gathered
   * from the provided list of repositories. Values found in repositories toward the front of the
   * list will override values in repositories toward the end of the list.
   *
   * @param componentName component name, not null
  ...
  public static SystemConfig create(
      String componentName, List<SystemConfigRepository> repositories) {
  ...
  private SystemConfig(String componentName, List<SystemConfigRepository> repositories) {
    Validate.notBlank(
        componentName, "SystemConfig cannot be created with a null or empty componentName");
    Objects.requireNonNull(repositories, "SystemConfig cannot be created with a null repository");

    LOGGER.info("system-configuration: creating system configuration");
  ...
  public URL getUrlOfComponent(String componentName) {
    return makeUrl(
        getValue(createKey(componentName, SystemConfig.HOST)),
