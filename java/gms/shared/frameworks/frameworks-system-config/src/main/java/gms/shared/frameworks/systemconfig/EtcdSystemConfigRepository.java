...
public class EtcdSystemConfigRepository implements SystemConfigRepository {
  ...
  private static final int BACKOFF_DELAY = 1;
  private static final int BACKOFF_MAX_DELAY = 30;
  ...
  @VisibleForTesting
  EtcdSystemConfigRepository(
      EtcdClientBuilder etcdClientBuilder, RetryPolicy<Optional<String>> etcdGetRetryPolicy) {
  ...
  static RetryPolicy<Optional<String>> defaultRetryPolicy() {
    return new RetryPolicy<Optional<String>>()
        ...
        .onRetry(
            event ->
                LOGGER.warn(
                    "Unable to retrieve System Config. Retrying...", event.getLastFailure()))
        .withBackoff(BACKOFF_DELAY, BACKOFF_MAX_DELAY, ChronoUnit.SECONDS)
        .withMaxAttempts(-1);
  }
  ...
  private Optional<String> fetchEtcd(String key)
      throws ExecutionException, TimeoutException, InterruptedException {
    LOGGER.debug("requesting kv for key {}", key);

    GetResponse response;
    try (var client = etcdClientBuilder.buildClient()) {
      CompletableFuture<GetResponse> responseFuture =
          client.getKVClient().get(ByteSequence.from(key, StandardCharsets.UTF_8));
      LOGGER.debug("retrieving response future");
      response = responseFuture.get(RESPONSE_TIMEOUT, TimeUnit.SECONDS);
    }
  ...
  public static class Builder {
    ...
    public Builder fromEnvironment() {
      ...
      this.username =
          Optional.ofNullable(System.getenv("ETCD_GMS_USER"))
              .orElseThrow(() -> new IllegalStateException("Unable to retrieve ETCD_GMS_USER"));
      // Access Etcd using provided read-only credentials
      this.password =
          Optional.ofNullable(System.getenv("ETCD_GMS_PASSWORD"))
