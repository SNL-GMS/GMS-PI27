...
public final class EnvironmentSystemConfigRepository implements SystemConfigRepository {
   ...
   *
   * <p>Since config values may contain the '.' and '-' characters which are unsupported as
   * environment variables names, we will look for an environment variable that matches a
   * transformed version of the key.
   *
   * <p>Specifically, the environment variable name will - Be prefixed with 'GMS_CONFIG_' - Be
   * converted to all capital letters. - Dashes will be translated to single-underscores. - Dots
   * will be translated to double-underscores.
   *
   * <p>For example, the key 'foo.bar-baz' will correspond to the environment variable
   * 'GMS_CONFIG_FOO__BAR_BAZ'.
   *
  ...
  public Optional<String> get(String key) {
    ...
    String environmentVariableName =
        "GMS_CONFIG_" + key.toUpperCase().replace("-", "_").replace(".", "__");
