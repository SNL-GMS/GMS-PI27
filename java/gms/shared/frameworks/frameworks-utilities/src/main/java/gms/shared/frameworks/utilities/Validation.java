...
public final class Validation {
   ...
   */
  public static <T, R> void throwForNonUnique(
      Collection<T> collection, Function<T, R> mapper, String description) {
    ...
    final List<String> duplicateStrings =
        collection.stream().collect(Collectors.groupingBy(mapper)).entrySet().stream()
  ...
  private static void throwForAll(String description, Collection<String> problems) {
    final var messageBuilder = new StringBuilder(description);
    problems.forEach(p -> messageBuilder.append("\t").append(p));

    throw new IllegalArgumentException(messageBuilder.toString());
  }

  /**
   * Addresses the LogForging sonarqube issue. Wrapped in it's own method so any additional
   * cleansing can be done without refactoring the rest of the code
   *
   * @param input string to be cleansed
   * @return cleansed string
   */
  public static String cleanseInputString(String input) {
