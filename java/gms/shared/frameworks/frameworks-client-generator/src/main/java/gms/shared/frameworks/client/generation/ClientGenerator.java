...
public final class ClientGenerator {
  ...
  public static <T> T createClient(Class<T> clientClass, SystemConfig systemConfig) {
    var sendRetryConfig =
        RetryConfig.create(
            ...
            ChronoUnit.valueOf(
                systemConfig.getValue("service-client-send-retry-delay-units").toUpperCase()),
            systemConfig.getValueAsInt("service-client-send-retry-max-attempts"));

    var upgradeRetryConfig =
        RetryConfig.create(
    ...
    return createClient(
        clientClass,
        systemConfig,
        ServiceClientJdkHttp.create(sendRetryConfig, upgradeRetryConfig));
  }
   ...
   */
  @VisibleForTesting
  static <T> T createClient(
      Class<T> clientClass, SystemConfig sysConfig, ServiceClientJdkHttp httpClient) {
  ...
  private static <T> Map<Method, PathMethod> pathMethodsByMethod(Class<T> clientClass) {
    return ServiceReflectionUtilities.findPathAnnotatedMethodsOnlyOrThrow(clientClass).stream()
        .collect(Collectors.toMap(PathMethod::getMethod, Function.identity()));
  }

  private static InvocationHandler handler(
      ServiceClientJdkHttp client, URL url, Duration timeout, Map<Method, PathMethod> pathMethods) {
    return (Object proxyObj, Method method, Object[] args) ->
        sendRequest(client, args[0], url, timeout, pathMethods.get(method));
  }

  private static Object sendRequest(
      ServiceClientJdkHttp httpClient,
      Object requestBody,
      URL url,
      Duration timeout,
    ...
    final Type responseType =
        ServiceReflectionUtilities.methodReturnsVoid(method)
            ? String.class
    ...
    return httpClient.send(
        ServiceRequest.from(
            ...
            timeout,
            responseType,
            pathMethod.getInputFormat(),
            pathMethod.getOutputFormat()));
  }

  private static URL appendToUrl(URL url, String path) throws MalformedURLException {
    return new URL(url.getProtocol(), url.getHost(), url.getPort(), url.getFile() + path, null);
  }
}
