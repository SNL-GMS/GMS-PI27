...
public class CoiEntityManagerFactory {
  ...
  private static final String UNABLE_TO_RETRIEVE_THE_SQL_PW_ERROR =
      "Unable to retrieve the sql password";
  ...
  public static EntityManagerFactory create(String unitName, SystemConfig config) {
    ...
    Map<String, String> propertiesOverrides =
        Map.of(
            HIBERNATE_CONNECTION_PROP_KEY,
            config.getValue(GMS_CONFIG_SQL_URL_KEY),
            HIBERNATE_CONNECTION_USERNAME_KEY,
            config.getValue(GMS_CONFIG_SQL_USERNAME_KEY),
            HIBERNATE_CONNECTION_PW_KEY,
            Optional.ofNullable(config.getValue(GMS_CONFIG_SQL_PW_KEY))
                .orElseThrow(() -> new IllegalStateException(UNABLE_TO_RETRIEVE_THE_SQL_PW_ERROR)),
            HIBERNATE_C3P0_POOL_SIZE_KEY,
            config.getValue(GMS_CONFIG_C3P0_POOL_SIZE_KEY),
            HIBERNATE_FLUSH_MODE,
            FLUSH_MODE_VALUE);
  ...
  public static EntityManagerFactory createElev(SystemConfig config) {
    return create(
        Map.of(
            ...
            config.getValue(GMS_CONFIG_SQL_URL_KEY),
            HIBERNATE_CONNECTION_USERNAME_KEY,
            config.getValue(GMS_CONFIG_SQL_ELEV_USERNAME_KEY),
            HIBERNATE_CONNECTION_PW_KEY,
