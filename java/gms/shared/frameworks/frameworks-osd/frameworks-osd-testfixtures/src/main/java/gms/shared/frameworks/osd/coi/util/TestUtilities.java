...
public class TestUtilities {
   ...
   */
  public static <T> void testSerialization(T object, Class<T> type) throws IOException {
  ...
  public static <T> void testSerialization(T object, Class<T> type, ObjectMapper objMapper)
    ...
    var serialized = objMapper.writeValueAsString(object);
    ...
    T deserialized = objMapper.readValue(serialized, type);
    Objects.requireNonNull(deserialized, "Expected deserialized object to be non-null");
    Validate.isTrue(
        object.equals(deserialized),
        "Expected deserialized object to equal original; original: "
            + object
            + " , deserialized: "
            + deserialized);
  }
  ...
  public static void checkClassEqualsAndHashcode(Class<?> clazz, boolean superClassEqualsSubclass) {
    ...
    var verifier = EqualsVerifier.forClass(clazz);
   ...
   */
  public static void checkAllConstructorsValidateNullArguments(
      Class<?> clazz, Object[][] validArguments) {

    Arrays.stream(validArguments).forEach(a -> checkConstructorValidatesNullArguments(clazz, a));

    Set<Constructor<?>> validatedConstructors =
    ...
    List<Constructor<?>> missingValidatedConstructors =
   ...
   */
  public static void checkConstructorValidatesNullArguments(
      Class<?> clazz, Object... validArguments) {

    Class<?>[] validArgumentTypes = convertToTypes(validArguments);
    var constructor = ConstructorUtils.getMatchingAccessibleConstructor(clazz, validArgumentTypes);
    if (constructor == null) {
      throw new IllegalArgumentException(
          "No constructor signature found for arguments: " + Arrays.toString(validArgumentTypes));
    }
    ...
    try {
      ...
      throw new IllegalArgumentException(
          "Could not call constructor with supposedly valid arguments", e);
    }
    ...
    for (var i = 0; i < validArgumentTypes.length; i++) {
      if (!constructorParameterTypes[i].isPrimitive()) {
        try {
          ...
          throw new IllegalStateException("Error creating test constructor instance", e);
        }
        // Exception wasn't thrown as expected.
        throw new IllegalStateException(
            "No validation for argument of type "
   ...
   */
  public static void checkStaticMethodValidatesNullArguments(
      Class<?> clazz, String methodName, Object... validArguments) throws IllegalAccessException {
    checkMethodValidatesNullArguments(
        clazz, null, methodName, Collections.emptyList(), validArguments);
  }
  ...
  public static void checkStaticMethodValidatesNullableArguments(
      Class<?> clazz,
      String methodName,
      List<Integer> nullableFieldIndices,
      Object... validArguments)
      throws IllegalAccessException {
    checkMethodValidatesNullArguments(
        clazz, null, methodName, nullableFieldIndices, validArguments);
  }
  ...
  public static void checkMethodValidatesNullArguments(
    ...
    checkMethodValidatesNullArguments(
        object.getClass(), object, methodName, Collections.emptyList(), validArguments);
  }
  ...
  public static void checkMethodValidatesNullableArguments(
      ...
      throws IllegalAccessException {
    checkMethodValidatesNullArguments(
        object.getClass(), object, methodName, nullableFieldIndices, validArguments);
  }
   ...
   *     ensure the method validates all arguments as non-null.
   */
  private static void checkMethodValidatesNullArguments(
      Class<?> clazz,
      Object object,
      String methodName,
      List<Integer> nullableFieldIndices,
    ...
    Class<?>[] validParameterTypes =
        Arrays.stream(validArguments).map(Object::getClass).toArray(Class[]::new);

    var method = MethodUtils.getMatchingAccessibleMethod(clazz, methodName, validParameterTypes);

    if (method == null) {
      throw new IllegalArgumentException(
          "No method signature found for method: "
              + methodName
              + ", arguments: "
    ...
    try {
      ...
      throw new IllegalArgumentException(
          "Could not call static method with supposedly valid arguments", e);
    }
