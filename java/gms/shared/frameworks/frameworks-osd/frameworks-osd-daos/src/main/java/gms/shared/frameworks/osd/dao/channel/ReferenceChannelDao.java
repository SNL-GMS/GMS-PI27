...
public class ReferenceChannelDao {
  ...
  @SequenceGenerator(
      name = "reference_channel_sequence",
      sequenceName = "reference_channel_sequence",
      allocationSize = 5)
  private long id;
  ...
  @Embedded private InformationSourceDao informationSource;
  ...
  public ReferenceChannelDao(ReferenceChannel channel) {
    ...
    this.aliases = channel.getAliases().stream().map(ReferenceAliasDao::new).toList();
  }
  ...
  public ReferenceChannel toCoi() {
    List<ReferenceAlias> aliasList = this.aliases.stream().map(ReferenceAliasDao::toCoi).toList();
