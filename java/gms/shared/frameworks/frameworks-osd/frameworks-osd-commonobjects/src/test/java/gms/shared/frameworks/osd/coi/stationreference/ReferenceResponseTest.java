...
class ReferenceResponseTest {
  ...
  void testSerialization() throws Exception {
    TestUtilities.testSerialization(
        StationReferenceTestFixtures.REFERENCE_RESPONSE, ReferenceResponse.class);
  }
  ...
  void testReferenceResponseCreate() {
    ...
    assertEquals(
        Optional.of(StationReferenceTestFixtures.REFERENCE_SOURCE_RESPONSE),
        response.getSourceResponse());
    assertEquals(StationReferenceTestFixtures.SYSTEM_TIME, response.getSystemTime());
    assertEquals(
        StationReferenceTestFixtures.REF_CALIBRATION_BHE_V_1, response.getReferenceCalibration());
    assertEquals(StationReferenceTestFixtures.FAP, response.getFapResponse().get());
  }
}
