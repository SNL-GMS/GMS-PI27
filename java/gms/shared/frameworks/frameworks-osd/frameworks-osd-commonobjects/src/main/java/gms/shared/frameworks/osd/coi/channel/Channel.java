...
public abstract class Channel {
  ...
  public static Channel from(
    ...
    final EnumMap<ChannelProcessingMetadataType, Object> processingMetadataEnumMap =
  ...
  interface Builder {
    ...
    Builder setProcessingMetadata(Map<ChannelProcessingMetadataType, Object> processingMetadata);
     ...
     *
     * <p>1. {@link Channel#getName()} and {@link Channel#getCanonicalName()} cannot be empty.
     *
     * <p>2. {@link Channel#getProcessingMetadata()} must have a value for {@link
     * ChannelProcessingMetadataType#CHANNEL_GROUP}
     *
     * <p>3. {@link Channel#getChannelOrientationType()} and {@link
     * Channel#getChannelOrientationCode()} must be consistent: code must match the type if type is
     * not {@link ChannelOrientationType#UNKNOWN}; code can never be whitespace.
     *
    ...
    default Channel build() {
      ...
      List<String> sortedInputs = getConfiguredInputs().stream().sorted().toList();
      setConfiguredInputs(sortedInputs);
      ...
      channel
          ...
          .forEach(
              ci ->
                  Validate.notBlank(ci, "none of the configured inputs should be an empty field"));
      ...
      Validate.isTrue(
          !Character.isWhitespace(channel.getChannelOrientationCode()),
          "channelOrientationCode cannot be whitespace");

      Validate.isTrue(
          orientationCodesMatch(
              channel.getChannelOrientationType(), channel.getChannelOrientationCode()),
     ...
     *     false otherwise.
     */
    private static boolean orientationCodesMatch(
        ChannelOrientationType orientationType, char orientationCode) {
     ...
     * @implNote first builds a temporary Channel, uses {@link
     *     ChannelNameUtilities#createShortName(Channel)} to create the raw Channel name, sets the
     *     name in the Builder, and then builds the final Channel. Could also implement the name
     *     operation using Channel.Builder instead of using a temporary Channel but that would
     *     require implementing getters for all of the builder fields used in the Channel name.
     */
    default Channel buildRaw() {
      ...
      final var PLACEHOLDER_NAME = "[placeholder]";
      final Channel unnamed =
          setConfiguredInputs(List.of())
              .setProcessingDefinition(Map.of())
              .setName(PLACEHOLDER_NAME)
              .setCanonicalName(PLACEHOLDER_NAME)
              .build();

      // Create the new Channel's name and canonicalName of: [STA].[GROUP].[FDSN_CHAN_NAME]
     ...
     *     Builder#build()} are unsatisfied.
     * @implNote first builds a temporary Channel, uses {@link
     *     ChannelNameUtilities#createName(Channel)} and {@link NameHash} to create the derived
     *     Channel name, sets the name in the Builder, and then builds the final Channel. Could also
     *     implement the name and hash operations using Channel.Builder instead of using a temporary
     *     Channel but that would require implementing getters for all of the builder fields used in
     *     the Channel name and hash.
     */
    ...
    private Channel buildWithName(String name) {
      return setName(name).setCanonicalName(name).build();
    }
  }
}
