...
public abstract class ReferenceChannel {
  ...
  public UUID getVersionId() {
    return UUID.nameUUIDFromBytes(
        (this.getName()
                ...
                + this.getElevation()
                + this.getDepth()
                + this.getVerticalAngle()
                + this.getHorizontalAngle()
                + this.getUnits()
                + this.getNominalSampleRate()
                + this.getActualTime())
  ...
  public abstract static class Builder {
    ...
    public ReferenceChannel build() {
      ...
      final var referenceChannel = autoBuild();

      Validate.notEmpty(referenceChannel.getName(), "name should not be an empty field");

      Validate.isTrue(
          !Character.isWhitespace(referenceChannel.getOrientationCode()),
          "orientationCode cannot be whitespace");

      Validate.isTrue(
          orientationCodesMatch(
              referenceChannel.getOrientationType(), referenceChannel.getOrientationCode()),
