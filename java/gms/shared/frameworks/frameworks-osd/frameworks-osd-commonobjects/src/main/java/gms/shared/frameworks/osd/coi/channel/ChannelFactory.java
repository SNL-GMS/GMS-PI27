...
public class ChannelFactory {
   ...
   */
  public static Channel rawFromReferenceChannel(
      ReferenceChannel referenceChannel, String stationName, String channelGroupName) {
    ...
    return Channel.builder()
        .setDescription(
            String.format(
                "Raw Channel created from ReferenceChannel %s with version %s",
        ...
        .setOrientationAngles(
            Orientation.from(
                referenceChannel.getHorizontalAngle(), referenceChannel.getVerticalAngle()))
  ...
  public static Channel filtered(Channel input, FilterDefinition filter) {
    ...
    metadata.put(
        ChannelProcessingMetadataType.FILTER_PASS_BAND_TYPE, filter.getFilterPassBandType());
    metadata.put(ChannelProcessingMetadataType.FILTER_LOW_FREQUENCY_HZ, filter.getLowFrequencyHz());
    metadata.put(
        ChannelProcessingMetadataType.FILTER_HIGH_FREQUENCY_HZ, filter.getHighFrequencyHz());
   ...
   * Channel#getOrientationAngles()} assigned based on the {@link
   * Channel#getChannelOrientationType()} of the input Channels, see {@link
   * ChannelFactory#beamOrientationAnglesFromType(ChannelOrientationType)}.
   *
   * <p>Precondition: Assumes all of the input Channels have the same: {@link Channel#getStation()},
  ...
  public static Channel beamed(Station station, List<Channel> inputs, BeamDefinition beam) {
    ...
    final EnumMap<ChannelProcessingMetadataType, Object> metadata =
        new EnumMap<>(ChannelProcessingMetadataType.class);
    metadata.putAll(canonical.getProcessingMetadata());

    // Warn if metadata already contains STEERING entries
    final Set<ChannelProcessingMetadataType> steeringKeys =
        Set.of(
    ...
    if (steeringKeys.stream().anyMatch(metadata::containsKey)) {
      LOGGER.warn(
          "Creating a beam derived Channel but the input Channels have already been steered to an"
              + " (azimuth, slowness). The canonical input channel is: {}",
          canonical);
    }
    ...
    final List<String> configuredInputs = inputs.stream().map(Channel::getName).toList();
    ...
    return canonical.toBuilder()
        ...
        .setProcessingMetadata(metadata)
        .setDescription(
            "Beam created for " + station.getName() + " with location " + station.getLocation())
        .buildDerived();
  }
  ...
  /**
   * Obtain a new derived {@link Channel} for applying the provided {@link FkSpectraDefinition} to
   * the provided input {@link Channel}.
   *
  ...
  public static Channel fk(Station station, List<Channel> inputs, FkSpectraDefinition fk) {
    ...
    final String description =
        String.format("FK channel created by applying fk to %s", configuredInputs);
