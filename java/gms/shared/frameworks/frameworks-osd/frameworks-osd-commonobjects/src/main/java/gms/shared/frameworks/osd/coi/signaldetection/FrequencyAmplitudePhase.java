 ...
 * frequency
 *
 * @deprecated in favor of {@link gms.shared.stationdefinition.coi.channel.FrequencyAmplitudePhase}
 */
@Deprecated(since = "PI24.5", forRemoval = false)
@AutoValue
...
public abstract class FrequencyAmplitudePhase {
  ...
  public abstract static class Builder {
    ...
    public FrequencyAmplitudePhase build() {
      ...
      Validate.isTrue(
          fap.getFrequencies().length == fap.getAmplitudeResponse().length
      ...
      IntStream.range(0, fapFrequencies.length)
          .forEach(
              i ->
                  sortedByFreqMap.put(
                      fapFrequencies[i],
                      List.of(
                          fap.getAmplitudeResponse()[i],
                          fap.getAmplitudeResponseStdDev()[i],
                          fap.getPhaseResponse()[i],
                          fap.getPhaseResponseStdDev()[i])));
  ...
  public AmplitudePhaseResponse getResponseAtFrequency(double freq) {
    if (this.getFrequencies().length == 0) {
      throw new IndexOutOfBoundsException(
          "Cannot retrieve response for frequency: " + freq + ". frequency list is empty");
    }
    ...
    if (this.getFrequencies().length < 2) {
      throw new IndexOutOfBoundsException(
          "Cannot retrieve response for frequency: "
              + freq
    ...
    checkState(
        freq >= f1 && freq <= f2,
        "Frequency does not fall within frequency bounds [%s, %s]",
        f1,
        f2);
    ...
    if (DoubleMath.fuzzyEquals(
        this.getAmplitudeResponseStdDev()[0],
        this.getAmplitudeResponseStdDev()[lastFreqIndex],
        TOLERANCE)) {
    ...
    if (DoubleMath.fuzzyEquals(
        this.getPhaseResponseStdDev()[0],
        this.getPhaseResponseStdDev()[lastFreqIndex],
        TOLERANCE)) {
    ...
    DoubleValue iamp = DoubleValue.from(interpAmplitude, stdDevA, this.getAmplitudeResponseUnits());
