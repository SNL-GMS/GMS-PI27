...
class ChannelNameUtilities {
   ...
   */
  static String createName(Channel channel) {
   ...
   * @return Channel short name, not null
   */
  static String createShortName(String station, String group, String fdsnChannelName) {
   ...
   */
  static String lookupChannelGroup(Map<ChannelProcessingMetadataType, Object> processingMetadata) {
   ...
   * ChannelProcessingMetadataType} to object map. Create attributes for as many processing
   ...
   *     empty but not null
   */
  private static String createAttributesForChannelName(
   ...
   * to object map if the map contains metadata for a filtered channel (see {@link
   * ChannelFactory#filtered(Channel, FilterDefinition)}). Returns an empty string if the metadata
   ...
   *     but not null
   */
  private static String createFilterAttributeForChannelName(
      Map<ChannelProcessingMetadataType, Object> metadata) {
    ...
    if (metadata.keySet().containsAll(required)) {
      return (Channel.COMPONENT_SEPARATOR
              ...
              + String.format(
                  "%.2f",
                  (double) metadata.get(ChannelProcessingMetadataType.FILTER_LOW_FREQUENCY_HZ))
              + "hz_"
              + String.format(
                  "%.2f",
                  (double) metadata.get(ChannelProcessingMetadataType.FILTER_HIGH_FREQUENCY_HZ))
              + "hz")
   ...
   * object map if the map contains metadata for a beam channel (see {@link
   * ChannelFactory#beamed(List, BeamDefinition)}). Returns an empty string if the metadata map does
   ...
   *     but not null
   */
  private static String createBeamAttributeForChannelName(
      Map<ChannelProcessingMetadataType, Object> metadata) {
    ...
    if (metadata.keySet().contains(ChannelProcessingMetadataType.BEAM_COHERENT)) {
      return (Channel.COMPONENT_SEPARATOR
              + "beam"
              + Channel.ATTRIBUTE_SEPARATOR
              + metadata.get(ChannelProcessingMetadataType.BEAM_COHERENT))
          .toLowerCase(Locale.ENGLISH);
    }
   ...
   * steered channel (see e.g. {@link ChannelFactory#beamed(List, BeamDefinition)}). Returns an
   * empty string if the metadata map does not contain steering metadata. If the attribute string is
   * not empty then it begins with {@link Channel#COMPONENT_SEPARATOR}.
   *
   * @param metadata processing metadata map of {@link ChannelProcessingMetadataType} to object, not
   *     null
   * @return String containing the beam specific attributes for a Channel name, potentially empty
   *     but not null
   */
  private static String createSteerAttributeForChannelName(
      Map<ChannelProcessingMetadataType, Object> metadata) {
    ...
    if (metadata.keySet().containsAll(required)) {
      return (Channel.COMPONENT_SEPARATOR
              ...
              + "az_"
              + String.format(
                  "%.3f", (double) metadata.get(ChannelProcessingMetadataType.STEERING_AZIMUTH))
              + "deg"
              + Channel.ATTRIBUTE_SEPARATOR
              + "slow_"
              + String.format(
                  "%.3f", (double) metadata.get(ChannelProcessingMetadataType.STEERING_SLOWNESS))
              + "s_per_deg")
          .toLowerCase(Locale.ENGLISH);
    }
  ...
  @AutoValue
  @JsonPropertyOrder({
    "sortedProcessingDefinition",
    "sortedConfiguredInputs",
    "channelDataType",
    "channelBandType",
  ...
  abstract static class NameHash {
    ...
    interface Builder {
      ...
      default NameHash build() {
        ...
        setConfiguredInputs(getConfiguredInputs().stream().sorted().toList());
