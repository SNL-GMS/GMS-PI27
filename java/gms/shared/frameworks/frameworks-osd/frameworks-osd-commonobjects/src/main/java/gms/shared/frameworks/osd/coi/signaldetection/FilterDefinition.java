...
public abstract class FilterDefinition {
  ...
  public static Builder firBuilder() {
    return builder().setFilterType(FilterType.FIR_HAMMING).setACoefficients(new double[] {1.0});
  }

  public static Builder iirBuilder() {
    return builder().setFilterType(FilterType.IIR_BUTTERWORTH);
  }
  ...
  public abstract static class Builder {
    ...
    public FilterDefinition build() {
      var filterDefinition = autoBuild();

      checkState(
          filterDefinition.getLowFrequencyHz() >= 0.0,
          "FilterDefinition requires low frequency >= 0.0");
      checkState(
          filterDefinition.getHighFrequencyHz() >= 0.0,
          "FilterDefinition requires high frequency >= 0.0");
      checkState(
          filterDefinition.getLowFrequencyHz() < filterDefinition.getHighFrequencyHz(),
      ...
      checkState(
          filterDefinition.getSampleRate() > 0.0, "FilterDefinition requires sampleRate > 0");
      checkState(
          filterDefinition.getSampleRateTolerance() >= 0.0,
          "FilterDefinition requires sampleRateTolerance >= 0");
      checkState(
          filterDefinition.getACoefficients().length > 0,
          "FilterDefinition requires at least 1 aCoefficient");
      checkState(
          filterDefinition.getBCoefficients().length > 0,
