...
public abstract class ReferenceNetwork {
  ...
  public abstract static class Builder {
    ...
    public ReferenceNetwork build() {
      var referenceNetwork = autoBuild();

      // Validate that entityId is valid
      final var expectedEntityId =
          UUID.nameUUIDFromBytes(referenceNetwork.getName().getBytes(StandardCharsets.UTF_16LE));
      Validate.isTrue(
          expectedEntityId.equals(referenceNetwork.getEntityId()),
          "Expected entityId to be "
              + expectedEntityId
              + " for name "
              + referenceNetwork.getName());

      // Validate that versionId is valid
      final var expectedVersionId =
          UUID.nameUUIDFromBytes(
              (referenceNetwork.getName()
                      + referenceNetwork.getOrganization()
                      + referenceNetwork.getRegion()
                      + referenceNetwork.getActualChangeTime())
                  .getBytes(StandardCharsets.UTF_16LE));
      Validate.isTrue(
          expectedVersionId.equals(referenceNetwork.getVersionId()),
