...
public abstract class Response<T> {
  ...
  private static final int LOWER_BOUND = 400;
  ...
  /**
   * Gets the body of the response. This should be present when the error message is empty, and this
   * should be empty when the error message is present.
   *
   * @return the body
   */
  ...
  /**
   * Gets the error message of this response. This should be empty when the body is present, and
  ...
  public static <T> Response<T> success(T body) {
    ...
    return new AutoValue_Response<>(Code.OK, Optional.of(body), Optional.empty());
  }

  /**
   * Creates a 'client error' response (400 BAD_REQUEST) with the provided error message and no
   * body. This can be used by route handler functions to conveniently return an error from a bad
   * client request.
   *
  ...
  /**
   * Creates a 'server error' response (500 INTERNAL_SERVER_ERROR) with the provided error message
   * and no body. This is generally used by this library when a route handler throws an exception.
   *
   * @param msg the error message
   * @return a Response
   */
  ...
  /**
   * Creates a generic 'error' response with the provided error message, no body, and the provided
   * status code
   *
  ...
  public static <T> Response<T> error(HttpStatus.Code statusCode, String msg) {
    ...
    Validate.isTrue(
        sc >= LOWER_BOUND && sc <= UPPER_BOUND,
        "For error, expected status code in range ["
            + LOWER_BOUND
            + ", "
            + UPPER_BOUND
            + "] but was "
            + sc);
    return new AutoValue_Response<>(statusCode, Optional.empty(), Optional.of(msg));
  }
}
