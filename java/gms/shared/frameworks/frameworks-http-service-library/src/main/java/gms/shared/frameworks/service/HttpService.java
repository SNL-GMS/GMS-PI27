...
public class HttpService {
  ...
  private static final int QUIET_PERIOD = 10;
  ...
  public void start() {
    ...
    String healthCheckEndpoint =
        "/" + definition.getContextRoot() + GmsCommonRoutes.HEALTHCHECK_PATH;
    String upgradeEndpoint =
        "/" + definition.getContextRoot() + GmsCommonRoutes.CONNECTION_UPGRADE_PATH;
    ...
    this.nettyService =
        configureServer()
            .route(
                (HttpServerRoutes routes) -> {
                  routes.get(
                      healthCheckEndpoint,
                      (req, res) ->
                          res.header(HttpHeaderNames.CONTENT_TYPE, APPLICATION_JSON)
                              .sendString(Mono.just("{\"aliveAt\":\"" + Instant.now() + "\"}")));
                  routes.get(
                      upgradeEndpoint,
                      (req, res) ->
                          res.header(HttpHeaderNames.CONTENT_TYPE, TEXT_PLAIN)
                              .sendString(
                                  Mono.just("Connection Upgraded to HTTP/2 at " + Instant.now())));
            ...
            .bindNow();
    isRunning = true;
    LOGGER.info("Service is now running on port {}", this.definition.getServerConfig().getPort());
  ...
  public void stop() {
    ...
    this.nettyService.disposeNow(Duration.ofSeconds(SHORT_TIMEOUT));
    // clean up http resources
    HttpResources.disposeLoopsAndConnectionsLater(
            Duration.ofSeconds(QUIET_PERIOD), Duration.ofSeconds(LONG_TIMEOUT))
        .block(Duration.ofSeconds(LONG_TIMEOUT));
    isRunning = false;
   ...
   *
   * <p>Invokes the {@link RequestHandler} operation on {@link Schedulers#boundedElastic()} since
   * the operation might be blocking, I/O bound, or otherwise slow to return.
   *
  ...
  private Publisher<Void> handleRequest(
      HttpServerRequest nettyRequest,
    ...
    final ObjectMapper deserializer =
        request.clientSentMsgpack()
            ? this.definition.getMsgpackMapper()
   ...
   */
  private Publisher<Void> writeOutResponse(
      HttpServerResponse nettyResponse, Response<?> routeResponse, boolean acceptsMsgPack) {
   ...
   */
  private static Publisher<Void> sendError(HttpServerResponse response, String errorMessage) {
    response.header(HttpHeaderNames.CONTENT_TYPE, TEXT_PLAIN);
    return response.sendString(Mono.just(errorMessage)).then();
  }
  ...
  private Publisher<Void> sendResponse(
    ...
    return sendMsgPack ? sendMsgPackStream(response, body) : sendJsonStream(response, body);
  }
  ...
  private Publisher<Void> sendMsgPackStream(HttpServerResponse response, Flux<?> body) {
    return response
        ...
        .sendByteArray(
            body.map(this::serializeObjectMsgPack).filter(Optional::isPresent).map(Optional::get));
  }
  ...
  private Publisher<Void> sendJsonStream(HttpServerResponse response, Flux<?> body) {
    return response
        ...
        .sendString(
            body.map(this::serializeObjectJson).filter(Optional::isPresent).map(Optional::get));
  }
  ...
  private Publisher<Void> sendResponse(
    ...
    return sendMsgPack ? sendMsgPackResponse(response, body) : sendJsonResponse(response, body);
  }
  ...
  private Publisher<Void> sendMsgPackResponse(HttpServerResponse response, Object body) {
    ...
    try {
      ...
      var messagePrefix = "Error sending MsgPack response";
      LOGGER.error(messagePrefix, e);
      return response.sendObject(Flux.just(messagePrefix, e.getMessage())).then();
    }
  }
  ...
  private Publisher<Void> sendJsonResponse(HttpServerResponse response, Object body) {
    ...
    try {
      ...
      var messagePrefix = "Error sending JSON response";
      LOGGER.error(messagePrefix, e);
      return response.sendObject(Flux.just(messagePrefix, e.getMessage())).then();
    }
  }
   ...
   *     result of handler.handle(request, deserializer)... if it throws an exception, a server
   *     error Response is returned.
   */
  private static Response<?> invokeHandler(
      RequestHandler<?> handler, Request request, ObjectMapper deserializer) {
    try {
