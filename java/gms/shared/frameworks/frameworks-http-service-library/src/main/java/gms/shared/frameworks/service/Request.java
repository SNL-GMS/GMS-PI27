...
/**
 * Interface for an HTTP request. You can retrieve things from it such as it's body or query params.
 * Intended to be implemented by using an underlying HTTP library.
 */
public interface Request {
  ...
  /**
   * Gets the body of the request as a raw byte array to be used in e.g. messagepack deserialization
   *
  ...
  private static boolean isHeaderMessagePackContentType(Request request, String header) {
    return parseContentTypeFromHeader(request, header)
        .filter(c -> (ContentType.MSGPACK == c || ContentType.MSGPACK_STREAM == c))
        .isPresent();
  }
  ...
  private static Optional<ContentType> parseContentTypeFromHeader(Request request, String header) {
    try {
      ...
      logger.warn(
          "ContentType can not be parsed.  Message is {}. Returning empty Optional",
          ex.getMessage());
