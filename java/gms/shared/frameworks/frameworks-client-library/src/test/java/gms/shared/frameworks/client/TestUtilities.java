...
class TestUtilities {
   ...
   * is serialized independently, so the serialization will contain a sequence of serialized objects
  ...
  static byte[] collectionToByteArray(Collection<?> collection, ContentType contentType) {
    ...
    final List<byte[]> responseByteArrays =
        collection.stream().map(serializer).flatMap(Optional::stream).collect(Collectors.toList());
  ...
  static Type getResponseType(Class<?> clazz, String methodName) {
    try {
      ...
      LOGGER.debug("Method '{}' not found for class '{}': {} ", methodName, clazz, e.getMessage());
    }
    return null;
  }
}
