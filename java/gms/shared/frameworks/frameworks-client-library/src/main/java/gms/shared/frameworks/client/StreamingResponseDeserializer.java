 ...
 */
final class StreamingResponseDeserializer {
   ...
   *
   * <p>The inputStream must contain individually serialized elements, e.g. a Flux of Strings should
   * be serialized as `"one" "two" ...` rather than `["one", "two", ...]`. If a portion of the
   * stream contains a serialization that can't be converted into an instance of the element type
   * then that portion of the stream is skipped, e.g. if a stream contains a Bar in what is supposed
   * to be a Flux of Foos, then an error is logged and the Bar is omitted from the Flux.
   *
   * <p>This operation will emit an exception to the Flux if any deserialization error other than an
   * invalid element occurs.
   *
   * <p>Closes the inputStream when done.
   *
   * @param inputStream the {@link InputStream} to read, not null. Will be closed by this operaiton.
   ...
   *     corresponds to a Flux but the element type is not-concrete (i.e. wildcard or
   *     type-parameter)
   */
    ...
    return Flux.push(
        (FluxSink<T> emitter) -> {
          try {
            ...
            // Can't recover from errors occurring in the parser unrelated to skipping an element
            // that couldn't be read or converted.
  ...
  /**
   * Reads a single object from the {@link JsonParser}, converts it to the provided {@link
   * JavaType}, and emits it with the provided {@link FluxSink}.
   *
