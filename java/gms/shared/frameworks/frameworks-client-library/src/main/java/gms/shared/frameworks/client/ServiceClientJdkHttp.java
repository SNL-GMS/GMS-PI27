...
public final class ServiceClientJdkHttp {
  ...
  private ServiceClientJdkHttp(
      HttpClient client, RetryConfig sendRetryConfig, RetryConfig upgradeRetryConfig) {
    this.httpClient = Objects.requireNonNull(client);
    this.sendRetryConfig = sendRetryConfig;
    this.upgradeRetryPolicy =
        upgradeRetryConfig
            .<HttpResponse<String>>toBaseRetryPolicy()
            .handle(
                List.of(
  ...
  public static ServiceClientJdkHttp create(
      RetryConfig sendRetryConfig, RetryConfig upgradeRetryConfig) {
   ...
   *     internal error
   * @throws BadRequest with body of response from server if server rejects the client request
    ...
    final RetryPolicy<T> retryPolicy =
        sendRetryConfig
            ...
            .onFailedAttempt(
                e ->
                    LOGGER.warn(
                        "Failed service request to {} with error {}, will try again...",
                        request.getUrl(),
                        e));
  ...
  <T, F> T sendSingleRequest(ServiceRequest request) {
    ...
    final var httpRequest = createHttpRequest(request);
    final HttpResponse<F> httpResponse =
        throwIfErrorResponse(sendHttp(httpRequest, responseProtocol.bodyHandler()));
    return tryDeserialize(httpResponse.body(), responseProtocol, request.getResponseType());
  }

  private static HttpRequest createHttpRequest(ServiceRequest request) {
    try {
  ...
  private static <F> BodyPublisher bodyProcessor(Object body, ContentType requestFormat) {
    final RequestContentProtocol<F> requestProtocol = ContentProtocols.from(requestFormat);
    return requestProtocol.bodyEncoder().apply(trySerialize(requestProtocol, body));
  }

  private static <F> F trySerialize(RequestContentProtocol<F> requestProtocol, Object data) {
    try {
  ...
  private <F> HttpResponse<F> sendHttp(HttpRequest request, BodyHandler<F> bodyHandler) {
    try {
      ...
      throw new ConnectionFailed(
          "Could not connect to " + request.uri() + " due to interrupt.", ex);
    } catch (Exception ex) {
  ...
  private HttpResponse<String> performHTTP2Upgrade(ServiceRequest request) {
    try {
      ...
      return Failsafe.with(
              upgradeRetryPolicy.onFailedAttempt(
                  e ->
                      LOGGER.warn(
                          "Failed service request to {} with error {}, will try again...",
                          request.getUrl(),
      ...
      throw new ConnectionFailed(
          "Could not upgrade connection to " + request.getUrl().getHost(), e);
    }
  }

  private static <T, F> T tryDeserialize(
      F data, ResponseContentProtocol<F> responseProtocol, Type type) {
    try {
  ...
  private static <X> HttpResponse<X> throwIfBadRequest(HttpResponse<X> response) {
