...
public class UserManager {
  ...
  private final UserPreferencesRepository userPreferencesRepositoryImpl;

  @Autowired
  public UserManager(UserPreferencesRepository userPreferencesRepositoryImpl) {
  ...
  public Optional<UserPreferences> getUserPreferencesByUserId(
      @io.swagger.v3.oas.annotations.parameters.RequestBody(
              description = "User id to retrieve preferences for")
          @RequestBody
          String userId) {
    // TODO: when switching to Spring we noticed the ui was sending quotes as in userId string ex:
    // "userId" we should fix the UI or accept json
