...
public class MagnitudeIdAmplitudeIdStationNameKey implements Serializable {
  ...
  private static final int MAX_STATION_NAME_LEN = 6;
  ...
  public static class Builder {
    ...
    public MagnitudeIdAmplitudeIdStationNameKey build() {
      ...
      if (magnitudeId != -1) {
        checkArgument(
            0 <= magnitudeId,
            "Magnitude Id is " + magnitudeId + ".  It must be greater than or equal to zero.");
      }

      // -1 indicates NA value
      if (amplitudeId != -1) {
        checkArgument(
            0 < amplitudeId, "Amplitude Id is " + amplitudeId + ".  It must be greater than zero.");
      }
      ...
      // NA not allowed
      checkArgument(
          stationName.length() <= MAX_STATION_NAME_LEN,
          "Station name is %s %s",
          stationName,
          DaoHelperUtility.createCharLengthString(MAX_STATION_NAME_LEN));
  ...
  public boolean equals(Object o) {
    ...
    return magnitudeId == that.magnitudeId
        && amplitudeId == that.amplitudeId
