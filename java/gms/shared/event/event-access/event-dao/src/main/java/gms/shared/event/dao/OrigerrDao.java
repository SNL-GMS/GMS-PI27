...
public class OrigerrDao {
  ...
  public static class Builder {
    ...
    private static final double CONFIDENCE_MIN = 0.5;
    ...
    /**
     * Validates and builds an {@link OrigerrDao} object
     *
     * @return {@link OrigerrDao} object
     */
    public OrigerrDao build() {
      ...
      verifyCovarianceMatrices();
      verifyAxis();
      verifyErrors();

      // NA not allowed
      checkArgument(
          (Math.abs(confidence) < TOLERANCE)
              || ((CONFIDENCE_MIN <= confidence) && (confidence <= CONFIDENCE_MAX)),
          "Confidence is "
              + confidence
              + ".  It must be 0.0 or in the range, ["
              + CONFIDENCE_MIN
              + ", "
              + CONFIDENCE_MAX
              + "].");
      ...
      if (commentId != NA_INT) {
        checkArgument(0 < commentId, "Comment Id is " + commentId + ".  It must not be negative.");
      }
    ...
    private void verifyCovarianceMatrices() {
      // -1.0 indicates NA value
      if (!isNA(covarianceMatrixSxx)) {
        checkArgument(
            0.0 < covarianceMatrixSxx,
            "covarianceMatrixSxx is "
                + covarianceMatrixSxx
                + DaoHelperUtility.createGreaterThanString(0));
      }

      // -1.0 indicates NA value
      if (!isNA(covarianceMatrixSyy)) {
        checkArgument(
            0.0 < covarianceMatrixSyy,
            "covarianceMatrixSyy is "
                + covarianceMatrixSyy
                + DaoHelperUtility.createGreaterThanString(0));
      }

      // -1.0 indicates NA value
      if (!isNA(covarianceMatrixSzz)) {
        checkArgument(
            0.0 < covarianceMatrixSzz,
            "covarianceMatrixSzz is "
                + covarianceMatrixSzz
                + DaoHelperUtility.createGreaterThanString(0));
      }

      // -1.0 indicates NA value
      if (!isNA(covarianceMatrixStt)) {
        checkArgument(
            0.0 < covarianceMatrixStt,
            "covarianceMatrixStt is "
                + covarianceMatrixStt
                + DaoHelperUtility.createGreaterThanString(0));
      }
    }
    ...
    private void verifyAxis() {
      // -1.0 indicates NA value
      if (!isNA(semiMajorAxisOfError)) {
        checkArgument(
            0.0 < semiMajorAxisOfError,
            "Semi-major axis of error ellipse is "
                + semiMajorAxisOfError
                + DaoHelperUtility.createGreaterThanString(0));
      }

      // -1.0 indicates NA value
      if (!isNA(semiMinorAxisOfError)) {
        checkArgument(
            0.0 < semiMinorAxisOfError,
            "Semi-minor axis of error ellipse is "
                + semiMinorAxisOfError
                + DaoHelperUtility.createGreaterThanString(0));
      }

      // -1.0 indicates NA value
      if (!isNA(strikeOfSemiMajorAxis)) {
        checkArgument(
            (0.0 <= strikeOfSemiMajorAxis) && (strikeOfSemiMajorAxis <= DEGREES_IN_CIRCLE),
            "Strike of major axis of error ellipse is %s %s",
            strikeOfSemiMajorAxis,
            DaoHelperUtility.createRangeStringDouble(0, DEGREES_IN_CIRCLE, '[', ']'));
      }
    }

    private void verifyErrors() {
      // -1.0 indicates NA value
      if (!isNA(standardErrorOfObservations)) {
        checkArgument(
            0.0 < standardErrorOfObservations,
            "Standard error of one observation is "
                + standardErrorOfObservations
                + DaoHelperUtility.createGreaterThanString(0));
      }
      ...
      if (!isNA(depthError)) {
        checkArgument(
            0.0 < depthError,
            "Depth error is " + depthError + DaoHelperUtility.createGreaterThanString(0));
      }

      // -1.0 indicates NA value
      if (!isNA(originTimeError)) {
        checkArgument(
            0.0 <= originTimeError,
            "Origin time error is " + originTimeError + ".  It must be not be negative.");
      }
    }

    private static boolean isNA(double value) {
      return Math.abs(value - NA) < TOLERANCE;
    }
  }
