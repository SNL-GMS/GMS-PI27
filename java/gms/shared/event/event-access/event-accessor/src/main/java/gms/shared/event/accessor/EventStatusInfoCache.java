...
/**
 * Provides the {@link IgniteCache} for storing {@link EventStatusInfo} objects mapped by {@link
 * WorkflowDefinitionId} and Event {@link UUID}. It also provides methods for adding and retrieving
...
public class EventStatusInfoCache {
  ...
  public static final CacheInfo EVENT_STATUS_INFO_CACHE =
      new CacheInfo("event-status-info-cache", CacheMode.PARTITIONED, CacheAtomicityMode.ATOMIC);

  private final IgniteCache<Pair<WorkflowDefinitionId, UUID>, EventStatusInfo>
      eventStatusInfoIgniteCache;
  ...
  /**
   * Gets the EventStatusInfo object associated with the provided {@link WorkflowDefinitionId} and
   * Event {@link UUID}. If no EventStatusInfo object exists, creates a new one with NOT_STARTED
   * status and an empty analyst list.
   *
   * @param stageId Workflow StageId
   * @param eventId Event UUID
   * @return An {@link Optional} containing the EventStatusInfo object mapped to the provided
   *     WorkflowDefinitionId and Event UUID.
   */
  public EventStatusInfo getOrCreateEventStatusInfo(WorkflowDefinitionId stageId, UUID eventId) {
    return getEventStatusInfo(stageId, eventId)
        .orElseGet(
            () -> {
              var newEventStatusInfo =
  ...
  /**
   * Adds the provided {@link EventStatusInfo} to the cache using the {@link WorkflowDefinitionId}
   * and Event {@link UUID} as keys.
   *
   * @param stageId Workflow StageId
   * @param eventId Event UUID
   * @param eventStatusInfo EventStatusInfo object to be added to cache
   */
  public void addEventStatusInfo(
    ...
    LOGGER.debug(
        "EventStatusInfoCache: StageId [{}], EventId [{}] has EventStatusInfo [{}]",
        stageId,
        eventId,
        eventStatusInfo);
  }
   ...
   * @param eventId Event UUID
   * @return An {@link Optional} containing the EventStatusInfo object mapped to the provided
   *     WorkflowDefinitionId and Event UUID. If no mapping exists for the provided keys, the
