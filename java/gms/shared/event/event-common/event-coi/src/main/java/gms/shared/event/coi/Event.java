...
public abstract class Event {
  ...
  public interface Builder {
    ...
    default Event build() {
      ...
      build
          ...
          .ifPresent(
              data ->
                  data.getEventHypotheses()
                      .forEach(
                          hypothesis ->
                              checkState(
                                  build.getId().equals(hypothesis.getId().getEventId()),
                                  "EventHypothesis has eventId not matching parent Event: event.id"
                                      + " %s, EventHypothesis.eventId %s",
                                  build.getId(),
                                  hypothesis.getId().getEventId())));
  ...
  public abstract static class Data {
    ...
    public abstract static class Builder {
      ...
      abstract ImmutableSet.Builder<PreferredEventHypothesis>
          preferredEventHypothesisByStageBuilder();
      ...
      public Event.Data.Builder addPreferredEventHypothesis(
      ...
      public abstract Event.Data.Builder setOverallPreferred(
          @Nullable EventHypothesis overallPreferred);
       ...
       * returns null if the MonitoringOrganization is not set. This is to support deserialization
       * of a faceted Location Solution using Jackson.
       *
       * @return Event.Data
       */
      @Nullable public Event.Data build() {
        ...
        checkState(
            !build.getMonitoringOrganization().isEmpty()
                && !build.getMonitoringOrganization().isBlank(),
            "Cannot create an event with empty or blank monitoringOrganization");
        checkState(
            !build.getEventHypotheses().isEmpty(), "An event must have at least one hypothesis");
        checkState(
            !build.getPreferredEventHypothesisByStage().isEmpty(),
            "An event must have at least one preferred event hypothesis");

        var buildEventHypothesisIds =
            build.getEventHypotheses().stream().map(EventHypothesis::getId).toList();
        var finalEventsNotPresent =
            build.getFinalEventHypothesisHistory().stream()
                .filter(eh -> !buildEventHypothesisIds.contains(eh.getId()))
                .map(EventHypothesis::getId)
                .toList();
        checkState(
            finalEventsNotPresent.isEmpty(),
            "Final Event Hypothesis history contains hypotheses not present in Event's hypotheses:"
                + " %s",
            finalEventsNotPresent);

        var preferredHypothesesNotPresent =
            build
                ...
                .map(
                    preferred ->
                        String.format(
                            "[Stage: %s, EH ID: %s]",
                            preferred.getStage(), preferred.getPreferred().getId()))
                .toList();
        checkState(
            preferredHypothesesNotPresent.isEmpty(),
            "Preferred event hypotheses by stage contains hypotheses not present in Event's"
                + " hypotheses: %s",
  ...
  public Optional<EventHypothesis> getFinal() {
    ...
    return Optional.of(finalEventHypothesisHistory.get(finalEventHypothesisHistory.size() - 1));
  }
}
