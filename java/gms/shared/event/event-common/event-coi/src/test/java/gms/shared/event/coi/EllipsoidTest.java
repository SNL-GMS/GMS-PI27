...
class EllipsoidTest {
  ...
  private static Stream<Arguments> ellipsoidValidationSource() {
    ...
    return Stream.of(
        ...
        Arguments.arguments(
            ScalingFactorType.COVERAGE, CONFIDENCE_LEVEL, K_WEIGHT, TIME_UNCERTAINTY, true),
        Arguments.arguments(
        ...
        Arguments.arguments(
            ScalingFactorType.K_WEIGHTED, CONFIDENCE_LEVEL, -1.0, TIME_UNCERTAINTY, true),
        Arguments.arguments(
            ScalingFactorType.K_WEIGHTED, CONFIDENCE_LEVEL, 0.0, TIME_UNCERTAINTY, false),
        Arguments.arguments(
            ScalingFactorType.K_WEIGHTED, CONFIDENCE_LEVEL, 0.1, TIME_UNCERTAINTY, false),
        // Confidence kWeight is 0.0
        Arguments.arguments(
            ScalingFactorType.CONFIDENCE, CONFIDENCE_LEVEL, 0.1, TIME_UNCERTAINTY, true),
        Arguments.arguments(
            ScalingFactorType.CONFIDENCE, CONFIDENCE_LEVEL, -0.1, TIME_UNCERTAINTY, true),
        Arguments.arguments(
            ScalingFactorType.CONFIDENCE, CONFIDENCE_LEVEL, 0.0, TIME_UNCERTAINTY, false),
        // Non-negative time uncertainty
        Arguments.arguments(
            SCALING_FACTOR_TYPE, CONFIDENCE_LEVEL, K_WEIGHT, Duration.ofNanos(1).negated(), true),
        Arguments.arguments(SCALING_FACTOR_TYPE, CONFIDENCE_LEVEL, K_WEIGHT, Duration.ZERO, false),
        Arguments.arguments(
            SCALING_FACTOR_TYPE, CONFIDENCE_LEVEL, K_WEIGHT, Duration.ofNanos(1), false));
  }

  @ParameterizedTest
  @MethodSource("ellipsoidValidationSource")
  void testEllipsoidValidation(
      ScalingFactorType scalingFactorType,
      double confidenceLevel,
      double kWeight,
      Duration TIME_UNCERTAINTY,
      boolean shouldFail) {
    var testEllipseBuilder =
        ELLIPSOID.toBuilder()
  ...
  void testSerialization() {
    JsonTestUtilities.assertSerializes(ELLIPSOID, Ellipsoid.class);
  }
}
