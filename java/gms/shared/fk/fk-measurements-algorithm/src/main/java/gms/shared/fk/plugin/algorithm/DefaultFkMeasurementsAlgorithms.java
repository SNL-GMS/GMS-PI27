 ...
 * given FK spectrum
 */
public final class DefaultFkMeasurementsAlgorithms {
  private static final double DOUBLE_EPSILON = 1e-10;
  private static final double FULL_ROTATION_RADIANS = 2 * Math.PI;
  private static final double FULL_ROTATION_DEGREES = 360;
  private static final double HALF_ROTATION_DEGREES = 180;
  public static final double GEO_POLAR_ROTATION = 90;

  /** This conversion term is calculated using the radius of the earth in kilometers (6371 km). */
  ...
  private DefaultFkMeasurementsAlgorithms() {
    // prevent instantiation
  }

  /**
   * The values in the fk spectrum parameter have been calculated with the FK stat function,
   * measuring the power of the beam at a provided slowness.
   *
   ...
   */
  public static double slownessOfIndex(
      double xSlowStart,
      double xSlowDelta,
      double ySlowStart,
      double ySlowDelta,
   ...
   */
  public static double slownessXComponent(
      double xSlowStart, double xSlowDelta, double xCoordinate) {
    return xSlowStart + xSlowDelta * xCoordinate;
  }
   ...
   */
  public static double slownessYComponent(
      double ySlowStart, double ySlowDelta, double yCoordinate) {
    return ySlowStart + ySlowDelta * yCoordinate;
  }

  /**
   * Calculate the azumith of the given (x, y) coordinates in the geographic polar system.
   *
   * @param xSlowStart the x coordinate for the start of the viewing window in the slowness space
   * @param xSlowDelta the change in the x coordinate for each array index
   * @param ySlowStart the y coordinate for the start of the viewing window in the slowness space
   ...
   */
  public static double azimuthOfIndex(
      double xSlowStart,
      double xSlowDelta,
      double ySlowStart,
      double ySlowDelta,
    ...
    if (DoubleMath.fuzzyEquals(xComponent, 0.0, DOUBLE_EPSILON)
        && DoubleMath.fuzzyEquals(yComponent, 0.0, DOUBLE_EPSILON)) {
      return 0;
    } else {
      /** Otherwise, return the azimuth in the geographic polar coordinate system. */
      double azimuth = -Math.atan2(xComponent, yComponent);

      if (azimuth < 0) {
        azimuth += FULL_ROTATION_RADIANS;
      }
  ...
  /**
   * Convert a given polar coordinate measured counterclockwise from the horizontal axis to the
   * geographical polar coordinate system measured clockwise from the vertical axis.
   *
   * <p>Note that this method does not make use of any modulus or remainder operations, as those
   * require integers.
   *
  ...
  /**
   * Calculates the uncertainty associated with the slowness vector of fk max.
   *
   ...
   * @param dbDownRadius maybe the radius of a circle?
   * @return the uncertainty associated with the slowness vector of the fk max (Units: sec/deg)
   */
  public static double slownessUncertainty(
      double highPass, double lowPass, double fstat, double dbDownRadius) {
    double centerFrequency = lowPass + (highPass - lowPass) / 2; // 5
  ...
  /**
   * Calculates the uncertainty associated with the azimuth of the maximum slowness vector from the
   * fk spectrum.
   *
   * @param slowness calculated slowness of the maximum of the fk spectrum (Units: sec/deg)
  ...
  public static double azimuthUncertainty(double slowness, double slownessUncertainty) {
    ...
    // Based on SME guidance, if the azimuth uncertainty resolves to a NaN, we should set it to
    // 180.0
    if (Double.isNaN(uncertainty)) {
      uncertainty = Math.toRadians(HALF_ROTATION_DEGREES);
    }
