...
public class GmsFrameworksConfiguration {
  ...
  private static final long TIMEOUT_VALUE = 60000;
  private static final long BACKOFF_INTERVAL = 500;
  ...
  @ConditionalOnProperty(
      prefix = "service.run-state.system-config",
      name = "state",
      havingValue = "service",
      matchIfMissing = true)
  @Primary
  ...
  public SystemConfig systemConfigLocal() {
    ...
    final var cleansedPath =
        PathValidation.getValidatedPath(
            Validation.cleanseInputString(systemConfigLocalPath), LOCAL_BASEPATH);

    var fileSystemConfigRepository =
        FileSystemConfigRepository.builder().setFilename(cleansedPath.toString()).build();

    return SystemConfig.create(
        gmsApplicationName,
        List.of(EnvironmentSystemConfigRepository.builder().build(), fileSystemConfigRepository));
  }

  @Bean("serviceBasedProcessingConfig")
  @ConditionalOnProperty(
      prefix = "service.run-state.processing-config",
      name = "state",
      havingValue = "service",
      matchIfMissing = true)
  public ConfigurationConsumerUtility processingConfigService(
    ...
    return ConfigurationConsumerUtility.builder(
            ClientGenerator.createClient(ConfigurationRepository.class, systemConfig))
        .retryConfiguration(retryConfig)
        .selectorCacheExpiration(systemConfig.getValueAsDuration("config-cache-expiration"))
        .selectorCacheMaxEntries(systemConfig.getValueAsLong("config-cache-max-entries"))
  ...
  public ConfigurationConsumerUtility processingConfigLocal() {
    ...
    var validatedPath =
        PathValidation.getValidatedPath(processingConfigDirLocalPath, LOCAL_BASEPATH);
    return ConfigurationConsumerUtility.builder(FileConfigurationRepository.create(validatedPath))
        .retryConfiguration(RetryConfig.create(INITIAL_DELAY, MAX_DELAY, ChronoUnit.SECONDS, 1))
        .build();
  }
  ...
  public RestTemplate restTemplate() {
    return new RestTemplateBuilder()
        .setConnectTimeout(Duration.ofMillis(TIMEOUT_VALUE))
        .setReadTimeout(Duration.ofMillis(TIMEOUT_VALUE))
  ...
  public RetryTemplate retryTemplate() {
    ...
    backOffPolicy.setInitialInterval(BACKOFF_INTERVAL);
  ...
  public OpenAPI openAPI() {
    ...
    return new OpenAPI()
        .info(new Info().title(gmsApi).description("Documentation for " + gmsApi).version("v0.0.1"))
        .openapi("3.0.1");
  }
