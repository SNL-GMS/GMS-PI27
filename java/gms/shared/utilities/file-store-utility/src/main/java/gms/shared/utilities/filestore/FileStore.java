...
public class FileStore {
  ...
  public FileStore(MinioClient minioClient) {
    ...
    this.jsonReader = ObjectMappers.jsonReader();
  }

  /**
   * Get a file given a FileDescriptor and type to deserialize to. The file is loaded from Minio and
  ...
  public <T> T findByFileDescriptor(FileDescriptor fileDescriptor, Class<T> type) {
    ...
    return this.findByFileDescriptor(
        fileDescriptor, getObjectMapperTransformer(fileDescriptor, type));
  }

  /**
   * Get a file given a FileDescriptor and file transformer. The file is loaded from Minio and
   * deserialized via the fileTransformer.
   *
   ...
   * @return The deserialized object.
   */
  public <T> T findByFileDescriptor(
      FileDescriptor fileDescriptor, FileTransformer<T> fileTransformer) {
   ...
   * @return A map that maps specific FileDescriptors to the deserialized objects that were found.
   */
  public <T> Map<FileDescriptor, T> findByKeyPrefix(
      FileDescriptor fileDescriptor, FileTransformer<T> fileTransformer) {
    ...
    var objectMetadatas =
        minioClient.listObjects(
            ListObjectsArgs.builder().bucket(fileDescriptor.getBucket()).prefix(prefix).build());
    ...
    return Streams.stream(objectMetadatas)
        ...
        .map(
            itemFileDiscriptor ->
                Map.entry(
                    itemFileDiscriptor, findByFileDescriptor(itemFileDiscriptor, fileTransformer)))
        .collect(Collectors.toMap(Entry::getKey, Entry::getValue));
  }

  /**
   * Helper method which returns a FileTransformer that simply uses the object mapper to
   * deserialize. Reduces some highly repetitive code.
   *
   * @param fileDescriptor file descriptor - its only use is to provide info if there is an
   *     exception.
   * @param type type to deserialize to, represented as a Java Class instance.
   * @param <T> Actual type to deserialize to.
   * @return A FileTransformer that use the jsonReader for this FileStore instance.
   */
  private <T> FileTransformer<T> getObjectMapperTransformer(
      FileDescriptor fileDescriptor, Class<T> type) {
    return (InputStream rawDataStream) -> {
      try {
        return jsonReader.readValue(rawDataStream, type);
      } catch (IOException e) {
        throw new FileStoreRuntimeException(fileDescriptor, e);
      }
    };
