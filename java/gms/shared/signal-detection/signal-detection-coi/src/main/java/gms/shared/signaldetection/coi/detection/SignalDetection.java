 ...
 * deleted during a particular processing stage (is deleted), in order to prevent their re-creation
 * in subsequent processing stages. Note that processing stage is tracked through the Creation Info
 * class attached to Signal Detection Hypothesis.
 */
@AutoValue
...
public abstract class SignalDetection {
  ...
  public static SignalDetection from(
      @JsonProperty("id") UUID id, @JsonProperty("data") Optional<SignalDetection.Data> data) {

    data.ifPresent(
        (Data dataVal) -> {
          for (SignalDetectionHypothesis sdh : dataVal.getSignalDetectionHypotheses()) {
            checkState(
                id.equals(sdh.getId().getSignalDetectionId()),
                "SignalDetectionHypothesis with UUID=%s "
                    + "cannot be assigned to SignalDetection with id=%s",
                sdh.getId().getSignalDetectionId(),
                id);
          }
        });

    return new AutoValue_SignalDetection.Builder().setId(id).setData(data).build();
  }

  public static SignalDetection createEntityReference(UUID id) {
    return new AutoValue_SignalDetection.Builder().setId(id).build();
  }

  public SignalDetection toEntityReference() {
    return new AutoValue_SignalDetection.Builder().setId(getId()).build();
  }
  ...
  public abstract static class Data {
    ...
    public abstract static class Builder {
      ...
      public abstract SignalDetection.Data.Builder setMonitoringOrganization(
      ...
      public SignalDetection.Data.Builder addSignalDetectionHypothesis(
        ...
        Objects.requireNonNull(
            signalDetectionHypothesis, "Cannot add null signal detection hypothesis");
      ...
      public SignalDetection.Data build() {
        List<Optional<?>> allFields =
            List.of(getMonitoringOrganization(), getSignalDetectionHypotheses(), getStation());
        ...
        if (numPresentFields == 0) {
          ...
          boolean onlyContainsDeleted = checkForDeletedOnly(data);

          checkArgument(
              (data.getSignalDetectionHypotheses().isEmpty() || !onlyContainsDeleted),
              "Cannot create a SignalDetection containing only deleted SignalDetectionHypotheses");

          long numHypothesesPopulated =
          ...
          if (numHypothesesPopulated == data.getSignalDetectionHypotheses().size()) {
            ...
            checkState(
                noParentCount <= 1,
                "SignalDetection can only contain one hypothesis without a parent hypothesis, found"
                    + " %d",
                noParentCount);
            ...
            for (var i = 0; i < data.getSignalDetectionHypotheses().size(); i++) {
              ...
              Optional<SignalDetectionHypothesisId> possibleParentID =
                  hypothesis.getParentSignalDetectionHypothesisId();
              checkState(
                  !possibleParentID.isPresent()
                      || hypothesisIds.indexOf(possibleParentID.get().getId()) < i,
                  "Cannot create a SignalDetection with a hypothesis whose parent was created after"
                      + " the child");
            }
          }
          ...
          throw new IllegalStateException(
              "Either all FacetedDataClass fields must be populated or none of them can be"
                  + " populated");
        }
      }

      /**
       * This method checks to see if the SignalDetectionHypotheses are all deleted, and returns
       * true, if so.
       *
       * @param data
       * @return true if all are deleted, false otherwise.
       */
      private static boolean checkForDeletedOnly(Data data) {
        for (SignalDetectionHypothesis sdh : data.getSignalDetectionHypotheses()) {
          ...
          if (sdh.getData().isPresent()) {
            if (sdhData.isPresent() && !sdhData.get().isDeleted()) {
              return false;
            }
          } else {
            return false;
  ...
  /**
   * Creates a new deleted {@link SignalDetectionHypothesis} for this SignalDetection by copying an
   * existing SignalDetectionHypothesis.
   *
   * @throws IllegalArgumentException if any parameters are null
   */
  public SignalDetection delete(UUID signalDetectionHypothesisIdToClone) {
    ...
    List<SignalDetectionHypothesis> signalDetectionHypothesesToClone =
        getDataOrThrow().getSignalDetectionHypotheses().stream()
            .filter(
                hypothesis -> signalDetectionHypothesisIdToClone.equals(hypothesis.getId().getId()))
            .toList();
    ...
    if (signalDetectionHypothesesToClone.size() != 1) {
      throw new IllegalArgumentException(
          signalDetectionHypothesesToClone.size()
              + " SignalDetectionHypotheses exist for id="
              + signalDetectionHypothesesToClone);
    }

    SignalDetectionHypothesis sdh = signalDetectionHypothesesToClone.get(0);
    var sdhData =
        sdh.getData().orElseThrow().toBuilder()
            .setDeleted(true)
            .setParentSignalDetectionHypothesis(Optional.ofNullable(sdh))
            .build();
    var sdh2 =
    ...
    List<SignalDetectionHypothesis> originalHypotheses =
        new ArrayList<>(getDataOrThrow().getSignalDetectionHypotheses());
