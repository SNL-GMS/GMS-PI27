...
public abstract class SignalDetectionBridgeDefinition {
  ...
  public interface Builder {
    ...
    Builder setDatabaseAccountByStage(
        ImmutableMap<WorkflowDefinitionId, String> databaseAccountByStage);

    default Builder setDatabaseAccountByStage(
    ...
    default SignalDetectionBridgeDefinition build() {
      ...
      Preconditions.checkState(
          !definition.getMonitoringOrganization().isEmpty(),
          "Monitoring organization cannot be empty");

      Preconditions.checkState(
          !definition.getMonitoringOrganization().isBlank(),
          "Monitoring organization cannot be blank");

      Preconditions.checkState(
          !definition.getOrderedStages().isEmpty(), "Ordered stages cannot be empty");

      List<WorkflowDefinitionId> workflowsMissingDatabase =
          definition.getOrderedStages().stream()
              .filter(stageId -> !definition.getDatabaseAccountByStage().containsKey(stageId))
              .toList();

      if (!workflowsMissingDatabase.isEmpty()) {
        throw new IllegalStateException(
            "Missing database account for stage(s): "
