...
public class SignalDetectionBridgeConfiguration {
  ...
  @Autowired
  public SignalDetectionBridgeConfiguration(
  ...
  public SignalDetectionBridgeDefinition getCurrentSignalDetectionBridgeDefinition() {
    var workflowDefinition =
        configurationConsumerUtility.resolve(
            ORDERED_STAGES_CONFIG, List.of(), WorkflowDefinition.class);
    ...
    var stagePersistenceDefinition =
        configurationConsumerUtility.resolve(
            ACCOUNTS_BY_STAGE_CONFIG, List.of(), StagePersistenceDefinition.class);

    var waveformTrimDefinition =
        configurationConsumerUtility.resolve(
            WAVEFORM_LEAD_LAG_CONFIG, List.of(), WaveformTrimDefinition.class);

    var monitoringOrganization =
        (String)
            configurationConsumerUtility
                .resolve(MONITORING_ORG_CONFIG, List.of())
