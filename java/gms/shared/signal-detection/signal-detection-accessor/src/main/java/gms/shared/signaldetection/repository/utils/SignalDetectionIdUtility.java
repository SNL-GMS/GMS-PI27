...
public class SignalDetectionIdUtility {
  ...
  private IgniteCache<AccountValue<Long>, UUID> sdhIdByAccountArrivalIdCache;
  private IgniteCache<UUID, AccountValue<Long>> accountArrivalIdBySdhIdCache;
  private IgniteCache<SignalDetectionHypothesisAssocIdComponents, UUID>
      assocIdComponentsSignalDetectionHypothesisIdMap;
  private IgniteCache<UUID, SignalDetectionHypothesisAssocIdComponents>
      signalDetectionHypothesisIdAssocIdComponentsMap;
  private IgniteCache<AmplitudeIdComponents, FeatureMeasurementIdComponents>
      amplitudeIdComponentsFeatureMeasurementIdComponentsMap;
  private IgniteCache<FeatureMeasurementIdComponents, AmplitudeIdComponents>
      featureMeasurementIdComponentsAmplitudeIdComponentsMap;
  ...
  SignalDetectionIdUtility(
      ...
      IgniteCache<AccountValue<Long>, UUID> arrivalIdComponentsSignalDetectionHypothesisIdMap,
      IgniteCache<UUID, AccountValue<Long>> signalDetectionHypothesisIdArrivalIdComponentsMap,
      IgniteCache<SignalDetectionHypothesisAssocIdComponents, UUID>
          assocIdComponentsSignalDetectionHypothesisIdMap,
      IgniteCache<UUID, SignalDetectionHypothesisAssocIdComponents>
          signalDetectionHypothesisIdAssocIdComponentsMap,
      IgniteCache<AmplitudeIdComponents, FeatureMeasurementIdComponents>
          amplitudeIdComponentsFeatureMeasurementIdComponentsMap,
      IgniteCache<FeatureMeasurementIdComponents, AmplitudeIdComponents>
          featureMeasurementIdComponentsAmplitudeIdComponentsMap) {

    this.aridSignalDetectionMap = aridSignalDetectionMap;
    this.signalDetectionAridMap = signalDetectionAridMap;
    this.sdhIdByAccountArrivalIdCache = arrivalIdComponentsSignalDetectionHypothesisIdMap;
    this.accountArrivalIdBySdhIdCache = signalDetectionHypothesisIdArrivalIdComponentsMap;
    this.assocIdComponentsSignalDetectionHypothesisIdMap =
        assocIdComponentsSignalDetectionHypothesisIdMap;
    this.signalDetectionHypothesisIdAssocIdComponentsMap =
        signalDetectionHypothesisIdAssocIdComponentsMap;
  ...
  /**
   * Find SignalDetectionHypothesis UUID for given Arrival Id and Stage Id, returns null if no value
   * is found
   *
   * @param arid Long Arrival Id
   ...
   */
  public UUID getSignalDetectionHypothesisIdForAridAndStageId(
      long arid, String legacyDatabaseAccountId) {
    var id = AccountValue.create(legacyDatabaseAccountId, arid);
    return sdhIdByAccountArrivalIdCache.get(id);
  }

  /**
   * Find Arrival Id and Stage Id for a given SignalDetectionHypothesis UUID, returns null if no
   * value is found
   *
   * @param uuid SignalDetectionHypothesis UUID
   * @return AccountValue class containing arid and stageid
   */
  public AccountValue<Long> getAccountArrivalIdFromSdhId(UUID uuid) {
    return accountArrivalIdBySdhIdCache.get(uuid);
  }
   ...
   * @param uuid SignalDetectionHypothesis UUID
   */
  public void addAridAndStageIdForSignalDetectionHypothesisUUID(
      long arid, String legacyDatabaseAccountId, UUID uuid) {
    Preconditions.checkNotNull(legacyDatabaseAccountId);
    Preconditions.checkNotNull(uuid);
    var id = AccountValue.create(legacyDatabaseAccountId, arid);
    sdhIdByAccountArrivalIdCache.put(id, uuid);
    accountArrivalIdBySdhIdCache.put(uuid, id);
  }
  ...
  /**
   * Find SignalDetectionHypothesis UUID for a given Arrival Id, Origin Id, and Stage Id, returns
   ...
   * @return SignalDetectionHypothesis UUID
   */
  public UUID getSignalDetectionHypothesisIdForAridOridAndStageId(
      long arid, long orid, String legacyDatabaseAccountId) {
    var id = SignalDetectionHypothesisAssocIdComponents.create(legacyDatabaseAccountId, arid, orid);
    return assocIdComponentsSignalDetectionHypothesisIdMap.get(id);
  }

  /**
   * Find Arrival Id, Origin Id, and Stage Id for a given SignalDetectionHypothesis UUID, returns
   * null if no value is found
   *
   * @param uuid SignalDetectionHypothesis UUID
   * @return {@link SignalDetectionHypothesisAssocIdComponents} containing arid, orid, and stage id
   */
  public SignalDetectionHypothesisAssocIdComponents
      getAssocIdComponentsFromSignalDetectionHypothesisId(UUID uuid) {
   ...
   * @param uuid SignalDetectionHypothesis UUID
   */
  public void addAridAndOridAndStageIdForSignalDetectionHypothesisUUID(
      long arid, long orid, String legacyDatabaseAccountId, UUID uuid) {
    Preconditions.checkNotNull(legacyDatabaseAccountId);
    Preconditions.checkNotNull(uuid);
    var id = SignalDetectionHypothesisAssocIdComponents.create(legacyDatabaseAccountId, arid, orid);
  ...
  /**
   * Find the UUID of SignalDetection for a given arid, creates and returns a new uuid if no value
   * is found in map
   *
   * @param arid Long Arrival Id
   * @return UUID
  ...
  /**
   * Find SignalDetectionHypothesis UUID for given Arrival Id and Stage Id, creates and returns a
   * new uuid if no value is found in map
   *
   * @param arid Long Arrival Id
   * @param legacyDatabaseAccountId String legacy account db id
   * @return SignalDetectionHypothesis UUID
   */
  public UUID getOrCreateSignalDetectionHypothesisIdFromAridAndStageId(
      long arid, String legacyDatabaseAccountId) {
  ...
  /**
   * Find SignalDetectionHypothesis UUID for given Arrival Id, Origin Id, and Stage Id, creates and
   * returns a new uuid if no value is found in map
   *
   * @param arid Long Arrival Id
   * @param orid Long Origin Id
   * @param legacyDatabaseAccountId String legacy account db id
   * @return SignalDetectionHypothesis UUID
   */
  public UUID getOrCreateSignalDetectionHypothesisIdFromAridOridAndStageId(
      long arid, long orid, String legacyDatabaseAccountId) {

    var uuid =
        getSignalDetectionHypothesisIdForAridOridAndStageId(arid, orid, legacyDatabaseAccountId);
  ...
  /**
   * Find Amplitude Id and Stage Id for a given SignalDetectionHypothesis UUID and Feature
   * Measurement type returns null if no value is found
   *
   * @param uuid SignalDetectionHypothesis UUID
   * @param featureMeasurementType FeatureMeasurementType
   * @return AmplitudeIdComponents class containing ampid and stageid
   */
  public AmplitudeIdComponents
      getAmplitudeIdComponentsFromSignalDetectionHypothesisIdAndFeatureMeasurementType(
          UUID uuid, FeatureMeasurementType featureMeasurementType) {

    return featureMeasurementIdComponentsAmplitudeIdComponentsMap.get(
        FeatureMeasurementIdComponents.create(uuid, featureMeasurementType));
  }

  /**
   * Find SignalDetectionHypothesis UUID and Feature Measurement type for a given Amplitude Id and
   * Stage Id returns null if no value is found
   *
   * @param ampid long Amplitude Id
   * @param legacyDatabaseAccountId String legacy db account id
   * @return FeatureMeasurementIdComponents class containing SignalDetectionHypothesis UUID and
   *     Feature Measurement
   */
  public FeatureMeasurementIdComponents getFeatureMeasurementIdComponentsFromAmpidAndStageId(
      long ampid, String legacyDatabaseAccountId) {

    return amplitudeIdComponentsFeatureMeasurementIdComponentsMap.get(
        AmplitudeIdComponents.create(legacyDatabaseAccountId, ampid));
  }

  /**
   * Add mapping of Signal detection hypothesis UUID and FeatureMeasurementType to and stage id and
   * ampid
   *
   * @param ampid Long Amplitude Id
   ...
   * @param featureMeasurementType FeatureMeasurementType
   */
  public void addAmpidAndStageIdForSignalDetectionHypothesisUUIDAndFeatureMeasurementType(
      long ampid,
    ...
    var featureMeasurementComponents =
        FeatureMeasurementIdComponents.create(uuid, featureMeasurementType);
    featureMeasurementIdComponentsAmplitudeIdComponentsMap.put(
        featureMeasurementComponents, ampidComponents);
    amplitudeIdComponentsFeatureMeasurementIdComponentsMap.put(
        ampidComponents, featureMeasurementComponents);
  }
}
