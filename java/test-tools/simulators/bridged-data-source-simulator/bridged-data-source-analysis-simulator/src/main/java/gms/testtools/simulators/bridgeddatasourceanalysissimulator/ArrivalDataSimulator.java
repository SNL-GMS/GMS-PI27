...
/**
 * The ArrivalDataSimulator is responsible for loading Arrival, Wftag, and Amplitude records into
 * the simulation database and removing these records from the simulation database.
 */
@Component
public class ArrivalDataSimulator implements AnalysisDataSimulator {
  ...
  private final Map<String, BridgedDataSourceRepository>
  ...
  public ArrivalDataSimulator(
      @Qualifier("arrivalDatabaseConnectorMap") Map<String, ArrivalDatabaseConnector> arrivalDatabaseConnectorMap,
      @Qualifier("amplitudeDatabaseConnectorMap") Map<String, AmplitudeDatabaseConnector> amplitudeDatabaseConnectorMap,
      @Qualifier("signalDetectionBridgedDataSourceRepositoryMap") Map<String, BridgedDataSourceRepository> signalDetectionBridgedDataSourceRepositoryMap,
      @Qualifier("wfTagDatabaseConnector") WftagDatabaseConnector wftagDatabaseConnector,
      @Qualifier("bridgedDataSourceWftagRepositoryJpa") BridgedDataSourceRepository wftagBridgedDataSourceRepository) {
    ...
    this.signalDetectionBridgedDataSourceRepositoryMap =
        signalDetectionBridgedDataSourceRepositoryMap;
   ...
   * @param copiedDataTimeShift - amount of time to shift data by to new ids generated for storing
   *     in the database
   */
  @Override
  public void loadData(
    ...
    List<ArrivalDao> arrivalDaos =
        arrivalSeedData.getArrivals().stream()
            .map(ArrivalDao::new)
            .map(
                arrivalDao -> {
                  long oldArid = arrivalDao.getId();
                  long newArid =
                      analysisDataIdMapper.getOrGenerate(AnalysisIdTag.ARID, oldArid, -1);
    ...
    List<WfTagDao> wfTagDaos =
        arrivalSeedData.getWfTags().stream()
            .map(WfTagDao::new)
            .map(
                wfTagDao -> {
    ...
    List<AmplitudeDao> amplitudeDaos =
        arrivalSeedData.getAmplitudeRecords().stream()
            ...
            .map(
                amplitudeDao -> {
                  ...
                  long newAmpid =
                      analysisDataIdMapper.getOrGenerate(AnalysisIdTag.AMPID, oldAmpid, -1);
                  long newArid =
                      analysisDataIdMapper.getOrGenerate(AnalysisIdTag.ARID, oldArid, -1);
  ...
  public void cleanup() {
    ...
    signalDetectionBridgedDataSourceRepositoryMap
        .values()
        .forEach(BridgedDataSourceRepository::cleanupData);
   ...
   */
  private void updateSeedDataMap(
      String stageId, Instant seedDataStartTime, Instant seedDataEndTime) {

    // iterate through the arrivalSeedDataMap by stage id and query for stage daos
    var arrivalSeedData = arrivalSeedDataMap.get(stageId);
    if (arrivalSeedData == null
        || !arrivalSeedData.getTimeRange().lowerEndpoint().equals(seedDataStartTime)
      ...
      List<ArrivalDao> initialArrivalSeedData =
          arrivalDatabaseConnectorMap
              .get(stageId)
              .findArrivalsByTimeRange(seedDataStartTime, seedDataEndTime);

      logger.info(
          "Arrival seed data size for stage {}: {}", stageId, initialArrivalSeedData.size());

      List<Long> arids =
          initialArrivalSeedData.stream()
              .map(ArrivalDao::getId)
              .distinct()
              .collect(Collectors.toList());

      // query for current stage wftags and remove any duplicates
      List<WfTagDao> initialWftagData = wftagDatabaseConnector.findWfTagsByArIds(arids);
      initialWftagData.removeIf(dao -> wftagKeys.contains(dao.getWfTagKey()));
      wftagKeys.addAll(
          initialWftagData.stream().map(WfTagDao::getWfTagKey).collect(Collectors.toSet()));

      logger.info("Wftag seed data size for stage {}: {}", stageId, initialWftagData.size());

      List<AmplitudeDao> initialAmplitudeData =
          amplitudeDatabaseConnectorMap.get(stageId).findAmplitudesByArids(arids);

      logger.info(
          "Amplitude seed data size for stage {}: {}", stageId, initialAmplitudeData.size());

      // update the current entry with new ArrivalSeedData object
      arrivalSeedDataMap.put(
          stageId,
          (ArrivalSeedData.create(
              initialArrivalSeedData,
  ...
  private void updateAridAndTime(
      ArrivalDao arrivalDao, Duration copiedDataTimeShift, long newArid) {
    arrivalDao
        .getArrivalKey()
        .setTime(arrivalDao.getArrivalKey().getTime().plus(copiedDataTimeShift));
