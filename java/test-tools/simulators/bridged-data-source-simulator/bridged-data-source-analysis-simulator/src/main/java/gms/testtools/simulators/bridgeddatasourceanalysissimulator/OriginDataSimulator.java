...
/**
 * The OriginDataSimulator is responsible for loading Origin, Event, EventControl, Assoc, ArInfo,
 * NetMag, StaMag records into the simulation database and removing these records from the
 * simulation database.
 */
@Component
public class OriginDataSimulator implements AnalysisDataSimulator {
  ...
  public OriginDataSimulator(
      @Qualifier("eventDatabaseConnectorMap") Map<String, EventDatabaseConnector> eventDatabaseConnectorMap,
      @Qualifier("originSimulatorDatabaseConnectorMap") Map<String, OriginSimulatorDatabaseConnector> originSimulatorDatabaseConnectorMap,
      @Qualifier("originErrDatabaseConnectorMap") Map<String, OriginErrDatabaseConnector> originErrDatabaseConnectorMap,
      @Qualifier("originBridgedDataSourceRepositoryMap") Map<String, BridgedDataSourceRepository> bridgedDataSourceRepositoryMap,
      @Qualifier("wfTagDatabaseConnector") WftagDatabaseConnector wftagDatabaseConnector) {
  ...
  public void loadData(
    ...
    var seedData =
        originSimulatorDatabaseConnector.findOriginDaosByPreciseTime(
    ...
    subscribeAndStore(
        stageId,
        transformOriginDaoFlux(originDaoFlux, getNewOriginDaoTransformer(analysisDataIdMapper))
            .collectList()
            .flux());
    ...
    subscribeAndStore(
        ...
        transformOriginDaoFlux(
            originDaoFlux,
            getOriginDaoEventDaoTransformer(analysisDataIdMapper, eventDatabaseConnector)));

    // Get, shift, and store WftagDao
    subscribeAndStore(
        stageId,
        transformOriginDaoFlux(
            originDaoFlux, getOriginDaoWftagDaoTransformer(analysisDataIdMapper)));
            ...
            .map(
                origerr ->
                    origerr.stream()
                        .map(
                            dao -> {
                              OrigerrDao.Builder newDao =
                                  OrigerrDao.Builder.initializeFromInstance(dao);
                              newDao.withOriginId(
                                  analysisDataIdMapper.getOrGenerate(
                                      AnalysisIdTag.ORID, dao.getOriginId(), -1));
                              return newDao.build();
                            })
  ...
  static Flux<OriginDao> getOriginDaoFlux(
      List<OriginDao> originDaoList, Duration copiedDataTimeShift) {
  ...
  private static Function<OriginDao, List<EventDao>> getOriginDaoEventDaoTransformer(
      AnalysisDataIdMapper analysisDataIdMapper, EventDatabaseConnector eventDatabaseConnector) {
    return originDao -> {
      Optional<EventDao> eventDaoSeed =
          eventDatabaseConnector.findEventById(originDao.getEventId());

      EventDao.Builder newDao = EventDao.Builder.initializeFromInstance(eventDaoSeed.get());
      newDao.withEventId(
          analysisDataIdMapper.getOrGenerate(AnalysisIdTag.EVID, originDao.getEventId(), -1));
      newDao.withPreferredOrigin(
          analysisDataIdMapper.getOrGenerate(AnalysisIdTag.ORID, originDao.getOriginId(), -1));
  ...
  private Function<OriginDao, List<WfTagDao>> getOriginDaoWftagDaoTransformer(
      AnalysisDataIdMapper analysisDataIdMapper) {

    return originDao -> {
      var evids = List.of(originDao.getEventId());

      var wftagDaos =
          wftagDatabaseConnector.findWfTagsByEvIds(evids).stream()
              .map(WfTagDao::new)
              .map(
                  wfTagDao -> {
                    long oldWfid = wfTagDao.getWfTagKey().getWfId();
                    long oldEvid = wfTagDao.getWfTagKey().getId();
                    long newWfid =
                        analysisDataIdMapper.getOrGenerate(AnalysisIdTag.WFID, oldWfid, -1);
                    long newEvid =
                        analysisDataIdMapper.getOrGenerate(AnalysisIdTag.EVID, oldEvid, -1);

                    wfTagDao.getWfTagKey().setWfId(newWfid);
                    wfTagDao.getWfTagKey().setId(newEvid);
  ...
  private static Function<OriginDao, Optional<EventControlDao>>
      getOriginDaoEventControlDaoTransformer(
          AnalysisDataIdMapper analysisDataIdMapper,
          OriginSimulatorDatabaseConnector originSimulatorDatabaseConnector) {

    return originDao -> {
      Optional<EventControlDao> optionalEventControlDao =
          originSimulatorDatabaseConnector.retrieveEventControlDaoByEventIdAndOriginId(
              new Builder()
      ...
      return optionalEventControlDao.map(
          eventControlDao -> {
            var newDaoBuilder =
                EventControlDao.Builder.initializeFromInstance(optionalEventControlDao.get());

            newDaoBuilder.withEventIdOriginIdKey(
  ...
  private static Function<OriginDao, List<AssocDao>> getOriginDaoAssocDaoTransformer(
    ...
    return originDao ->
        originSimulatorDatabaseConnector
                ...
                assocDao -> {
                  ...
                  newDao.withId(
                      new AridOridKey.Builder()
                          .withOriginId(
                              analysisDataIdMapper.getOrGenerate(
                                  AnalysisIdTag.ORID, assocDao.getId().getOriginId(), -1))
        ...
        originSimulatorDatabaseConnector
            ...
            .map(
                arInfoDao -> {
                  ...
                  newDao.withOriginIdArrivalIdKey(
                      new OriginIdArrivalIdKey.Builder()
                          .withOriginId(
                              analysisDataIdMapper.getOrGenerate(
                                  AnalysisIdTag.ORID, arInfoDao.getOriginId(), -1))
                          .withArrivalId(newArid)
                          .build());
            ...
            .map(
                netMagDao -> {
                  ...
                  return newDao
                      .withOriginId(
                          analysisDataIdMapper.getOrGenerate(
                              AnalysisIdTag.ORID, netMagDao.getOriginId(), -1))
                      .withEventId(
                          analysisDataIdMapper.getOrGenerate(
                              AnalysisIdTag.EVID, netMagDao.getEventId(), -1))
                      .withMagnitudeId(
                          analysisDataIdMapper.getOrGenerate(
                              AnalysisIdTag.MBID, netMagDao.getMagnitudeId(), -1))
                      .build();
                })
            .collect(Collectors.toList());
  }
                  ...
                  return newDao
                      .withMagnitudeIdAmplitudeIdStationNameKey(
                          new MagnitudeIdAmplitudeIdStationNameKey.Builder()
                              .withAmplitudeId(
                                  analysisDataIdMapper.getOrGenerate(
                                      AnalysisIdTag.AMPID, staMagDao.getAmplitudeId(), -1))
                              .withMagnitudeId(
                                  analysisDataIdMapper.getOrGenerate(
                                      AnalysisIdTag.MBID, staMagDao.getMagnitudeId(), -1))
                              .withStationName(staMagDao.getStationName())
                              .build())
                      .withOriginId(
                          analysisDataIdMapper.getOrGenerate(
                              AnalysisIdTag.ORID, staMagDao.getOriginId(), -1))
                      .withEventId(
                          analysisDataIdMapper.getOrGenerate(
                              AnalysisIdTag.EVID, staMagDao.getEventId(), -1))
                      .withArrivalId(newArid)
                      .build();
                })
            .collect(Collectors.toList());
  ...
  private static <T> Flux<T> transformOriginDaoFlux(
      Flux<OriginDao> originDaoFlux, Function<OriginDao, T> transformer) {
