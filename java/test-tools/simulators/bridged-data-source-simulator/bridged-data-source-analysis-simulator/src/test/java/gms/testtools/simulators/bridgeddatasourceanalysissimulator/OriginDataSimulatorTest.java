...
class OriginDataSimulatorTest {
  ...
  @Mock OriginSimulatorDatabaseConnector originSimulatorDatabaseConnector;
  @Mock EventDatabaseConnector eventDatabaseConnector;
  @Mock OriginErrDatabaseConnector originErrDatabaseConnector;
  @Mock BridgedDataSourceRepository bridgedDataSourceRepository;
  ...
  @Captor ArgumentCaptor<List<?>> listArgumentCaptor;
  ...
  void testLoadDataForOrigin() {
    ...
    Mockito.when(
        ...
        .thenReturn(List.of(OriginDataSimulatorTestFixture.happyOriginDaoBuilder.build()));

    Mockito.when(
            originSimulatorDatabaseConnector.retrieveEventControlDaoByEventIdAndOriginId(any()))
        .thenReturn(Optional.of(OriginDataSimulatorTestFixture.happyEventControlDao.build()));

    Mockito.when(originSimulatorDatabaseConnector.retrieveAssocDaoListFromOriginId(anyLong()))
        .thenReturn(List.of(OriginDataSimulatorTestFixture.happyAssocDaoBuilder.build()));

    Mockito.when(originSimulatorDatabaseConnector.retrieveArInfoDaoListForOriginId(anyLong()))
        .thenReturn(List.of(OriginDataSimulatorTestFixture.happyArInfoDaoBuilder.build()));

    Mockito.when(originSimulatorDatabaseConnector.retrieveNetMagDaoListForOriginId(anyLong()))
        .thenReturn(List.of(OriginDataSimulatorTestFixture.happyNetMagDaoBuilder.build()));

    Mockito.when(originSimulatorDatabaseConnector.retrieveStamagDaoListForOriginId(anyLong()))
        .thenReturn(List.of(OriginDataSimulatorTestFixture.happyStaMagDaoBuilder.build()));

    Mockito.when(wftagDatabaseConnector.findWfTagsByEvIds(List.of(1L)))
        .thenReturn(List.of(OriginDataSimulatorTestFixture.happyWftagDao));
    ...
    verify(bridgedDataSourceRepository, times(36)).store(listArgumentCaptor.capture());
    ...
    assertEquals(
        OriginDataSimulatorTestFixture.happyModifiedOriginDaoBuilder.build(),
        daoTypes.get(0).get(0));

    assertEquals(
        OriginDataSimulatorTestFixture.happyEventDaoBuilder.build(), daoTypes.get(1).get(0));

    assertEquals(
        OriginDataSimulatorTestFixture.happyWftagDao.getWfTagKey(),
        ((WfTagDao) daoTypes.get(2).get(0)).getWfTagKey());

    assertEquals(
        OriginDataSimulatorTestFixture.happyOrigerrDaoBuilder.build(), daoTypes.get(3).get(0));

    assertEquals(
        OriginDataSimulatorTestFixture.happyEventControlDao.build(), daoTypes.get(4).get(0));

    assertEquals(
        OriginDataSimulatorTestFixture.happyAssocDaoBuilder.build(), daoTypes.get(5).get(0));

    assertEquals(
        OriginDataSimulatorTestFixture.happyArInfoDaoBuilder.build(), daoTypes.get(6).get(0));

    assertEquals(
        OriginDataSimulatorTestFixture.happyNetMagDaoBuilder.build(), daoTypes.get(7).get(0));

    assertEquals(
        OriginDataSimulatorTestFixture.happyStaMagDaoBuilder
  ...
  void testGetOriginDaoFlux() {
    ...
    var result =
        OriginDataSimulator.getOriginDaoFlux(inputOriginDaoList, Duration.ofSeconds(90000))
            .collectList()
            .block();
