...
public class BridgedDataSourceStationRepositoryJpa extends BridgedDataSourceRepositoryJpa
  ...
  private static final Logger logger =
      LoggerFactory.getLogger(BridgedDataSourceStationRepositoryJpa.class);
  ...
  private static final String DATA_ERR = ".cleanupData() error.";

  public BridgedDataSourceStationRepositoryJpa(EntityManagerFactory entityManagerFactory) {
  ...
  public void cleanupData() {
    ...
    var errMessage = this.getClass().getSimpleName() + DATA_ERR;
                          ...
                          siteChanDaos.stream()
                              .map(
                                  key ->
                                      builder.and(
                                          builder.equal(
                                              id.get(STATION_CODE), key.getId().getStationCode()),
                                          builder.equal(
                                              id.get(CHANNEL_CODE), key.getId().getChannelCode())))
                              .toArray(Predicate[]::new))));

          List<SiteChanDao> siteChansToUpdate =
              entityManager.createQuery(siteChanQuery).getResultList();

          // map sta,chan pair to list of corresponding sitechans from db
          siteChansToUpdate.forEach(
              siteChan -> staChanToSiteChanMap.put(mapSiteChanDaoToString(siteChan), siteChan));

          List<SiteChanDao> siteChansToStore =
              siteChanDaos.stream()
                  .map(
                      siteChanDao -> {
                        Set<SiteChanDao> correspondingSiteChans =
                            staChanToSiteChanMap.get(mapSiteChanDaoToString(siteChanDao));
                        var siteChanKey = siteChanDao.getId();

                        if (correspondingSiteChans.isEmpty()) {
                          logger.warn(
                              "SiteChan with sta {} and chan {} is not currently in simulation"
                                  + " data, so new sitechan cannot be added.",
                              siteChanKey.getStationCode(),
                              siteChanKey.getChannelCode());
                          return Optional.<SiteChanDao>empty();
                        }

                        List<SiteChanDao> sameOnDates =
                            correspondingSiteChans.stream()
                                .filter(
                                    correspondingSiteChan ->
                                        correspondingSiteChan
                                            .getId()
                                            .getOnDate()
                        ...
                        if (!sameOnDates.isEmpty()) {
                          logger.warn(
                              "SiteChan with sta {} and chan {}  and ondate {} has same ondate as"
                                  + " sitechan in simulation data, so it cannot be added.",
                              siteChanKey.getStationCode(),
                              siteChanKey.getChannelCode(),
                        ...
                        correspondingSiteChans.forEach(
                            correspondingSiteChan -> {
                              ...
                              .map(
                                  key ->
                                      builder.and(
                                          ...
                                          builder.and(
                                              builder.equal(
                                                  fromSite.get(REF_STA),
                                                  key.getReferenceStation()))))
                              .toArray(Predicate[]::new))));

          List<SiteDao> sitesToUpdate = entityManager.createQuery(siteQuery).getResultList();

          sitesToUpdate.forEach(
              siteDao -> staRefStaToSiteMap.put(mapSiteDaoToString(siteDao), siteDao));

          List<SiteDao> sitesToStore =
              siteDaos.stream()
                  .map(
                      siteDao -> {
                        Set<SiteDao> correspondingSites =
                            staRefStaToSiteMap.get(mapSiteDaoToString(siteDao));
                        var siteKey = siteDao.getId();
                        if (correspondingSites.isEmpty()) {
                          logger.warn(
                              "Site with sta {} and refsta {} is not currently in simulation data,"
                                  + " so new site cannot be added.",
                              siteKey.getStationCode(),
                              siteDao.getReferenceStation());
                          return Optional.<SiteDao>empty();
                        }

                        List<SiteDao> sameOndates =
                            correspondingSites.stream()
                                .filter(
                                    correspondingSite ->
                                        correspondingSite
                                            .getId()
                                            ...
                                            .truncatedTo(ChronoUnit.DAYS)
                                            .equals(
                                                siteKey.getOnDate().truncatedTo(ChronoUnit.DAYS)))
                                .collect(Collectors.toList());

                        if (!sameOndates.isEmpty()) {
                          logger.warn(
                              "Site with sta {} and refsta {} has same ondate as site in simulation"
                                  + " data, so site cannot be added.",
                              siteKey.getStationCode(),
                              siteDao.getReferenceStation());
                          return Optional.<SiteDao>empty();
                        }
