...
class ChannelSegmentCreatorTest {
  ...
  private static final Channel CHANNEL =
      Channel.createVersionReference("TEST.TEST1.BHZ", Instant.now());
  ...
  void testChannelSegmentCreator() {
    ...
    List<ChannelSegment<Waveform>> waveformChannelSegment =
        CreatorFactory.getChannelSegmentCreatorInstance()
            .getChannelSegments(channelList, START_TIME, endTime);
    ...
    assertEquals(
        ...
        channelSegmentResult.getId().getEndTime().toEpochMilli(),
        MILLISECOND_DELTA);
  ...
  void testEmptyChannelList() {
    ...
    final List<ChannelSegment<Waveform>> result =
        CreatorFactory.getChannelSegmentCreatorInstance()
