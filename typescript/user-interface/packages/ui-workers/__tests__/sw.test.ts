...
jest.mock('@gms/common-util', () => {
  const actualCommonUtil = jest.requireActual('@gms/common-util');
  return {
    ...actualCommonUtil,
    IS_NODE_ENV_PRODUCTION: true
  };
});
...
describe('Service Worker entrypoint', () => {
  ...
  describe('handleFetch', () => {
    it(`if refreshed while there is a worker waiting, sends the waiting worker a ${skipWaitingMessage} message`, () => {
      ...
      const postMessageSpy = jest.spyOn(self.registration.waiting as unknown as any, 'postMessage');
      handleFetch({
    ...
    it('responds with the request from the cache if there is a match', () => {
      ...
      jest.spyOn(self.clients, 'matchAll').mockImplementation(async () => {
    ...
    it('responds with the initial request if there is a cache miss', () => {
      ...
      globalAny.fetch = jest.fn(async () => Promise.resolve(mockResponse));
