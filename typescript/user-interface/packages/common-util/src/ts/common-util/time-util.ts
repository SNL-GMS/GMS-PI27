 ...
 */
export const dateStringIsValid = (str: string): boolean =>
  moment(str, [
    DATE_FORMAT,
    TIME_FORMAT,
 ...
 */
export function toEpochSeconds(str: string | undefined | null): number {
...
export const truncateTimeUnits = (timeUnits: TimeUnits, maxTimeUnits?: number): TimeUnits => {
  ...
  const millisecondThreshold = 6;
  const increaseThresholdBy = getIndexOfFirstSignificantTimeUnit(timeUnits);
  let threshold;
  if (maxTimeUnits) {
    if (maxTimeUnits > 0) {
      threshold = maxTimeUnits + increaseThresholdBy;
    } else {
  ...
  return {
    ...
    milliseconds: threshold >= millisecondThreshold ? timeUnits.milliseconds : 0
  };
};
