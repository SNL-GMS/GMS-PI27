...
import type { SystemEventType } from '@gms/common-model/lib/common/types';
 ...
 */
export const getWsConnection = (): WebSocket | undefined => socketConnection;
 ...
 */
export const getSubscriptionMetadata = (): Immutable.List<SystemEventType> =>
...
const sendHeartbeat = (): void => {
  if (socketConnection && connectionCompleted) {
    try {
      const heartbeat = Immutable.List([{ eventType: 'heartbeat' }]);
      socketConnection.send(JSON.stringify(heartbeat));
    } catch (error) {
      logger.error('Failed to send subscription heartbeat to System Event gateway', error);
    }
  }
};
...
export const sendSubscriptionEventTypes = (): void => {
  ...
  if (socketConnection && connectionCompleted && !typesToSubscribe.isEmpty()) {
    try {
...
export const onMessage: (e: MessageEvent) => void = e => {
  try {
    ...
    } else if (systemEvent.type === 'heartbeat') {
      logger.debug(`Subscription received heartbeat`);
      sendHeartbeat();
    } else if (subscribers.has(systemEvent.type)) {
      const data = deserializeTypeTransformer(systemEvent.data);
      const subscriberEventType = subscribers.get(systemEvent.type);
      if (subscriberEventType) {
        subscriberEventType.forEach(subscriber => {
...
export const addSubscriber = <T>(
  ...
  if (subscribers.has(eventType)) {
    const subscriberEventType = subscribers.get(eventType);
    if (subscriberEventType) {
      subscribers = subscribers.set(eventType, subscriberEventType.set(subscriber.id, subscriber));
    }
  } else {
...
export const removeSubscriber = (id: string, eventType: string): void => {
  if (subscribers.has(eventType)) {
    let subscriberEventType = subscribers.get(eventType);
    if (subscriberEventType) {
      subscribers = subscribers.set(eventType, subscriberEventType.delete(id));
    }

    subscriberEventType = subscribers.get(eventType);
    if (subscriberEventType && subscriberEventType.size <= 0) {
