...
import { selectOpenActivityNames } from '../../state/workflow/selectors';
 ...
 */
export const selectFilterLists = (state: AppState): FilterTypes.FilterList[] | undefined =>
...
export const selectPreferredFilterListsByActivity = (
  state: AppState
): FilterTypes.FilterListActivity[] | undefined =>
  signalEnhancementConfigurationApiSlice.endpoints.getFilterListsDefinition.select()(state).data
 ...
 * */
export const selectPreferredFilterListForActivity: (state: AppState) => string | undefined =
  createSelector(
    [selectPreferredFilterListsByActivity, selectOpenActivityNames],
    (preferredFilterListByActivity, openActivityNames) => {
      const activity = openActivityNames?.find(
        aName => preferredFilterListByActivity?.find(pf => pf.workflowDefinitionId.name === aName)
      );
      if (!activity) {
        logger.error(
