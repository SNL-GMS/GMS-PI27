...
const isStageIntervalMatched = (s1: WorkflowTypes.StageInterval, s2: WorkflowTypes.StageInterval) =>
  s1.intervalId.definitionId.name === s2.intervalId.definitionId.name &&
  s1.stageMode === s2.stageMode &&
  s1.intervalId.startTime === s2.intervalId.startTime &&
 ...
 */
export const updateStageIntervals =
  (stageIntervals: WorkflowTypes.StageInterval[]) =>
  (draft: MaybeDrafted<StageIntervalList>): void => {
    if (draft && stageIntervals) {
      stageIntervals.forEach(stageInterval => {
        // If no map or map entry not found
        if (!draft.find(si => si.name === stageInterval.intervalId.definitionId.name)) {
          logger.warn(
            `Query cache does not have stage ${stageInterval.intervalId.definitionId.name}`
          );
          return;
        }

        // Get the Stage Interval list for the relevant Stage
        const cacheStageIntervals: WorkflowTypes.StageInterval[] | undefined = draft.find(
          si => si.name === stageInterval.intervalId.definitionId.name
        )?.value;
        let newData = [stageInterval];
        if (cacheStageIntervals) {
          newData = mergeStageIntervals(cacheStageIntervals, stageInterval);
        }

        // update the data with the updated values
        const draftRecord = draft.find(
          si => si.name === stageInterval.intervalId.definitionId.name
        );
        if (draftRecord) draftRecord.value = newData;
      });
    }
  };
      ...
      stageIntervalsByIdAndTime: build.query<StageIntervalList, StageIntervalsByIdAndTimeParams>({
        ...
        async onCacheEntryAdded(
          ...
          const onOpen = (e: Event, isReconnect: boolean): void => {
            ...
            if (isReconnect) {
              dispatch(analystActions.setEffectiveNowTime());
            }
          };
 ...
 */
export const produceCleanUp =
  (staleCleanUpBoundary: number | undefined) =>
  (draft: MaybeDrafted<StageIntervalList>): void => {
    if (staleCleanUpBoundary !== undefined && draft) {
      draft.forEach((entry, index) => {
        ...
        entry.value.forEach(v => {
          if (v.intervalId.startTime >= v.endTime) {
...
export type UpdateStageIntervalStatusMutationFunc = MutationTrigger<
  MutationDefinition<UpdateStageIntervalStatusParams, BaseQueryFn, never, void, 'workflowApi'>
>;
...
export type UpdateActivityIntervalStatusMutationFunc = MutationTrigger<
  MutationDefinition<UpdateActivityIntervalStatusParams, BaseQueryFn, never, void, 'workflowApi'>
>;
