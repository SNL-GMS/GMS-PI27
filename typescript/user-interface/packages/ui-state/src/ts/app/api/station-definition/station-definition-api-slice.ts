...
import { setDefaultFilterForStationsIfFilterIsNotSet } from '../../state/analyst/operations';
import { config } from './endpoint-configuration';

const logger = UILogger.create(
  'GMS_STATION_DEFINITION_API_SLICE',
  process.env.GMS_STATION_DEFINITION_API_SLICE
);
...
interface OSDStation extends Omit<StationTypes.Station, 'relativePositionsByChannel'> {
  relativePositionChannelPairs?: {
    ...
    channel: FacetedTypes.EntityReference<'name'>;
  }[];
}
  ...
  endpoints(build) {
    return {
      ...
      getStations: build.query<StationTypes.Station[], StationsProps>({
        query: (args: StationsProps) => ({
          requestConfig: {
            ...config.stationDefinition.services.getStations.requestConfig,
            data: {
              effectiveTime: args.effectiveTime,
              stationNames: sortBy(args.stationNames), // always use a sorted list
              facetingDefinition: args.facetingDefinition
            }
          }
        }),
        ...
        async onQueryStarted(args, { dispatch, queryFulfilled }) {
          try {
            const { data: stations } = await queryFulfilled;
            dispatch(setDefaultFilterForStationsIfFilterIsNotSet(stations));
          } catch (error) {
