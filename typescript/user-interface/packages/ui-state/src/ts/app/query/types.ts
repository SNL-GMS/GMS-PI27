...
/** Used for checking error messages in queries */
...
export interface AsyncFetchHistoryEntry<T> {
  ...
  /** the number of attempts the query has been issued */
  attempts: number;
  /** the time in epoch seconds of when the last attempt was completed */
  time: number;
  /**
...
interface BaseAsyncFetchRequest<T> {
...
export type AsyncFetchResult<T> =
  | Override<
      BaseAsyncFetchRequest<T>,
      | Omit<BaseAsyncFetchRequest<T>, 'error'>
      | ({
          isLoading: false;
          isError: false;
        } & Required<Pick<BaseAsyncFetchRequest<T>, 'data'>>)
    >
  | Override<
      BaseAsyncFetchRequest<T>,
      | Omit<BaseAsyncFetchRequest<T>, 'data'>
      | {
          isLoading: true;
          isError: false;
        }
  ...
  | Override<
      ...
      | ({
          isLoading: false;
          isError: true;
        } & Required<Pick<BaseAsyncFetchRequest<T>, 'error'>>)
    >;
...
/**
 * The fetch history status, excludes any data.
 */
export type FetchHistoryStatus = Omit<AsyncFetchResult<never>, 'data'>;
