...
export const buildChannelSegmentBounds = ({
  ...
  throw new Error('amplitudeMax must be > -Infinity and amplitudeMin must be < Inifinity');
};
...
export function scaleToPositionBufferIndex(
  ...
  if (startTimeSecs && endTimeSecs && startTimeSecs > endTimeSecs)
    throw new Error(
...
export const calculateDataSegmentBounds = async (
  dataByClaim: WeavessTypes.DataClaimCheck,
  startTimeSecs?: number,
  endTimeSecs?: number
