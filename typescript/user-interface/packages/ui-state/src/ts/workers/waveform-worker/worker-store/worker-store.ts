...
export class WorkerStore<StoredType = unknown> {
  ...
  private readonly STORE_TO_INDEXED_DB = !!process.env.GMS_STORE_TO_INDEXED_DB || false;
  ...
  public readonly store = async (
    ...
    if (isPromise(valueOrPromise)) {
      ...
      this.storeToMemAndDB({ id, value: valueOrPromise });
    }
  };
   ...
   */
  public readonly retrieve = async (id: string, depth = 0): Promise<StoredType | undefined> => {
    ...
    if (result) {
      this.inMemResultMap = this.inMemResultMap.set(id, result.value);
      return result.value;
    }
    ...
    return new Promise<StoredType | undefined>(resolve => {
      ...
      setTimeout(
        ...
        depth ** 2 * 100
      ); // back off with increasing amount of time
    });
  };

  /**
   * Resolves all promises in memory and then returns the store of all key/value pairs
   *
   * @returns a key/value record of all ids mapped to their stored values.
   */
  public readonly exportStore = async (): Promise<Record<string, StoredType>> => {
    // resolve all promises
    await Promise.all(
      this.promiseMap
        .keySeq()
        .toArray()
        .map(async id => this.resolvePromiseAndStore(id))
    );
    if (this.promiseMap.size > 0) {
      throw new Error('Cannot export store. Promises were not all resolved.'); // ! Perhaps this needs to be recursive?
    }
    return this.inMemResultMap.toObject();
  };

  /**
   * Import a new store. This overwrites the old store. There is no way to recover it!
   *
   * @param newStore the store that should replace the old store
   */
  public readonly importStore = async (newStore: Record<string, StoredType>): Promise<void> => {
    await this.cleanup();
    this.inMemResultMap = Immutable.Map(newStore);
  };
  ...
  private readonly storeToDB = ({ id, value }: { id: string; value: StoredType }) => {
    const timeoutHandle = setTimeout(async () => {
      await this.resultDB.results.put({ id, value });
    }, 0);
    this.timeoutMap = this.timeoutMap.set(`db-${id}`, timeoutHandle);
  };
   ...
   */
  private readonly storeToMemAndDB = ({ id, value }: { id: string; value: StoredType }) => {
    this.inMemResultMap = this.inMemResultMap.set(id, value);
    if (this.STORE_TO_INDEXED_DB) this.storeToDB({ id, value });
  };
  ...
  private readonly resolvePromiseAndStore = async (id: string): Promise<StoredType> => {
    ...
    const value = await p;
    if (!value) {
      throw new Error('Unable to resolve undefined promise');
    }
    this.storeToMemAndDB({ id, value });
