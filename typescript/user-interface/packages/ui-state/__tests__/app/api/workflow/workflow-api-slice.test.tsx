...
describe('workflow api slice', () => {
  ...
  it('hook updates activity interval status', async () => {
    const params: UpdateActivityIntervalStatusParams = {
      userName: 'test',
      time: 100,
      activityIntervalId: { definitionId: { name: 'test' }, startTime: 3 },
      stageIntervalId: { definitionId: { name: 'test' }, startTime: 3 },
        ...
        updateStageIntervals([
          {
            ...
            intervalId: {
              ...sampleData.interactiveAnalysisStageInterval.intervalId,
              definitionId: {
                ...sampleData.interactiveAnalysisStageInterval.intervalId.definitionId,
                name: 'unknown'
              }
            }
          }
        ...
        updateStageIntervals([
          {
            ...
            intervalId: {
              ...sampleData.interactiveAnalysisStageInterval.intervalId,
              startTime: sampleData.interactiveAnalysisStageInterval.intervalId.startTime + 5
            },
  ...
  describe('can produce cleaned up intervals', () => {
    ...
    it('test shifting boundary - with multiples', () => {
      ...
      itr1.forEach(i => {
        ...
        expect(Math.min(...flatMap(result[0].value.map(e => e.endTime || 0)))).toBeGreaterThan(
          staleCleanUpBoundary
        );
        expect(Math.min(...flatMap(result[1].value.map(e => e.endTime || 0)))).toBeGreaterThan(
          staleCleanUpBoundary
        );
      });
    });
