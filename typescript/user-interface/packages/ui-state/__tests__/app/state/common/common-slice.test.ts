...
describe('state workspace common', () => {
  ...
  describe('actions', () => {
    ...
    it('should set the unique component id', () => {
      const id = Date.now();

      const expectedAction: Redux.AnyAction = {
        type: commonSlice.actions.setUniqueComponent.type,
        payload: {
          name: IanDisplays.WAVEFORM,
          id
        }
      };
      expect(commonSlice.actions.setUniqueComponent(expectedAction.payload)).toEqual(
        expectedAction
      );
      store.dispatch(commonSlice.actions.setUniqueComponent(expectedAction.payload));
