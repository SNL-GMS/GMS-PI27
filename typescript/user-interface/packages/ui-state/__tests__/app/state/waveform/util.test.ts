...
import {
  ...
  getStationName,
  getVisibleChannels,
  getVisibleStationNamesFromStationVisibilityChangesDictionary,
  getVisibleStations,
  isChannelVisible,
  isStationExpanded,
  isStationVisible,
...
describe('Waveform state utils', () => {
  ...
  describe('StationVisibilityChangesDictionary', () => {
    ...
    it('can find if station is visible in dictionary', () => {
      expect(isStationVisibleInChangesDict(sDict, 'AAK')).toBe(true);
    });
    it('can find visible stations list in dictionary', () => {
      expect(getVisibleStations(sDict, [station])).toMatchSnapshot();
      expect(getVisibleStations(sDict, [])).toHaveLength(0);
    });

    it('can find names of visible stations from stationVisibilityChangesDictionary', () => {
      const nameArray = getVisibleStationNamesFromStationVisibilityChangesDictionary(sDict);
      expect(nameArray).toHaveLength(Object.keys(sDict).length);
      expect(nameArray[0]).toBe('AAK');
    });
