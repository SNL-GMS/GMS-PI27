...
export class KafkaConsumer extends AbstractKafka {
  ...
  private readonly configureEventListeners = () => {
    this.consumer.on(this.consumer.events.CONNECT, (e: InstrumentationEvent<unknown>) => {
    ...
    this.consumer.on(this.consumer.events.DISCONNECT, (e: InstrumentationEvent<unknown>) => {
      const info = `${this.convertTimestamp(
        e.timestamp
    ...
    this.consumer.on(this.consumer.events.STOP, (e: InstrumentationEvent<unknown>) => {
      const info = `${this.convertTimestamp(
        e.timestamp
  ...
  /** Handles connecting to the KAFKA consumer */
  private readonly connectAndReconnect = async (): Promise<void> => {
   ...
   */
  private readonly handleMessages = async (
    topic: string,
    ...
    new Promise(resolve => {
      if (
        ...
        const callback = this.kafkaConsumerCallbacks.get(topic);
        if (callback) {
  ...
  private readonly run = async (): Promise<void> => {
    await this.connectAndReconnect();
    ...
    await this.consumer.run({
      eachBatch: async ({ batch, resolveOffset }) => {
        ...
        try {
          ...
          await this.handleMessages(batch.topic, messages);
        } catch (e) {
