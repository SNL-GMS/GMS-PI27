...
import { setAppAuthenticationStatus } from '@gms/ui-state';
...
> => {
  try {
    ...
    if (profile.username) {
      ...
      return setAppAuthenticationStatus(authenticationStatus);
    }
    throw new Error('Keycloak: username is not defined');
  } catch {
    throw new Error('Keycloak: failed to load user profile');
  }
};
/**
