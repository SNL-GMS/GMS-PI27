...
export const getDisplaysFromContent = (content: GLConfigResult): string[] => {
  const displays = content.content ? flatMap(content.content, getDisplaysFromContent) : [];
  return content.component ? [content.component] : displays;
};
/**
...
export const getDisplaysFromConfig = (glConfig: GLConfigResult): string[] => {
  ...
  return flatMap(content, getDisplaysFromContent);
};
...
export const getOpenDisplays = (gl: GoldenLayout): string[] => {
  if (gl) {
    ...
    return getDisplaysFromConfig(currentConfig);
  }
  return [];
};
...
export const getClosedDisplays = (gl: GoldenLayout, glComponents: GLMap): string[] => {
  if (gl) {
    ...
    return xor(allKeys, openDisplayKeys);
  }
  return [];
};
 ...
 */
export const createOpenDisplayFunction =
  (gl: GoldenLayout, glConfig: GLConfig) =>
...
export const createCloseDisplayFunction =
  (gl: GoldenLayout) =>
  (componentKey: string): void => {
    gl.root.contentItems.forEach(contentItem =>
      removeDisplaysFromContent(contentItem, componentKey)
    );
  };
...
export const uniqueLayouts = (
  layouts: Readonly<UserProfileTypes.UserLayout[]>,
...
export const GMS_LAYOUT_KEY_PREFIX = 'gms-analyst-ui-layout';

/**
 * Creates a string key identifying a layout.
 *
 * This is used as the key to store a layout in localStorage
 *
 * @param layoutName the name of the layout for which to get the key
 * @returns a key identifying the layout
 */
export function getLayoutKey(layoutName: string | undefined) {
  return `${GMS_LAYOUT_KEY_PREFIX}-${layoutName}`;
}
