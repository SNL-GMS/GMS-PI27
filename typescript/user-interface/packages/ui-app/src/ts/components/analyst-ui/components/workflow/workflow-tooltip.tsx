...
import { LabelValue, Tooltip2Wrapper } from '@gms/ui-core-components';
...
export interface TooltipPanelProps {
  ...
  readonly tooltipRef: React.MutableRefObject<HTMLElement | null> | null;
...
function InternalTooltipPanel(props: TooltipPanelProps) {
  ...
  const staleness = `${status} ${isStale ? '(Stale)' : ''}`;
  return (
...
function InternalTooltip(props: React.PropsWithChildren<TooltipProps>) {
  ...
  const startTime: string = secondsToString(interval.intervalId.startTime);
