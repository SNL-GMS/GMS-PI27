...
import type { SiteColumn, SiteConfigurationRow, SiteConfigurationTableProps } from './types';

function formatRows(sites: ChannelTypes.ChannelGroup[]): SiteConfigurationRow[] {
  if (sites) {
    return sites.map((chan: ChannelTypes.ChannelGroup, idx: number): SiteConfigurationRow => {
      return {
        id: `${idx}`,
        ...
        type: toSentenceCase(getChannelGroupTypeForDisplay(chan.type))
      };
    });
  }
  return [];
 ...
 */
export function SiteConfigurationTable(props: SiteConfigurationTableProps) {
  ...
  React.useEffect(() => {
    return delayExecutionReturnClearTimeout(() => {
      if (tableRef?.current && tableRef.current.getTableApi()) {
        tableRef.current
          ?.getTableApi()
          ?.forEachNode(node =>
    ...
    >
      ...
      <div className={classNames(['station-properties-table__wrapper'])}>
        <Table<SiteConfigurationRow, unknown>
          ...
          onRowClicked={(event: RowClickedEvent) => onRowSelection(event)}
