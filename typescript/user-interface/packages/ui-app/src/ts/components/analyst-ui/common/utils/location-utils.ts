...
export function getAzimuthValues(
  ...
  const fmValue = SignalDetectionTypes.Util.findAzimuthFeatureMeasurementValue(
    SignalDetectionTypes.Util.getCurrentHypothesis(sd.signalDetectionHypotheses).featureMeasurements
  );
...
export function getSlownessValues(
  ...
  const fmValue = SignalDetectionTypes.Util.findSlownessFeatureMeasurementValue(
    SignalDetectionTypes.Util.getCurrentHypothesis(sd.signalDetectionHypotheses).featureMeasurements
  );
...
export function convertSignalDetectionToSnapshotWithDiffs(
  ...
  return {
    ...
    phase: SignalDetectionTypes.Util.findPhaseFeatureMeasurementValue(
    ...
    deletedOrUnassociated: undefined
  };
}
/**
...
export function getSnapshotsWithDiffs(
  ...
  // ones in locationSnaps but no associated are added to master with "associatedDiff" true and deleted/unnassoc true
...
export function generateSnapshotsForLatestLocation(
  ...
  const preferredLocationSolution = latestLSS.locationSolutions.find(
  ...
  const locationSolution =
    selectedLocationSolutionId && latestLSS ? preferredLocationSolution : undefined;
...
export function generateSnapshotsForPreviousLocation(
  ...
  const maybeSelectedLSS =
    openEvent.currentEventHypothesis.eventHypothesis.locationSolutionSets.find(
...
export function updateLocBehaviorFromTableChanges(
  ...
  const changedTo = change === DefiningChange.CHANGED_TO_TRUE ? true : newBehavior.defining;
  newBehavior.defining = change === DefiningChange.CHANGED_TO_FALSE ? false : changedTo;
  return newBehavior;
}
