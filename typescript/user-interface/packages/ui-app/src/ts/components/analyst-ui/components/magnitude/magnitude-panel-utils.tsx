...
export const getAmplitudesByStation = (
  ...
  stationMagnitudeSDData.forEach(sdData => {
    const magnitudeTypes = magnitudeTypesForPhase.get(sdData.phase);
    if (magnitudeTypes) {
...
/**
 * Sets focus to magnitude
 */
export function setFocusToMagnitude(): void {
  const magnitudePanel = document.getElementsByClassName('magnitude-wrapper');
  const magnitudePanelElement = magnitudePanel[0] as HTMLElement;
  magnitudePanelElement?.focus();
}
