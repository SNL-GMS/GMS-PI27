...
export class LocationHistory extends React.Component<LocationHistoryProps, LocationHistoryState> {
  ...
  public render(): JSX.Element {
    ...
    const mainTableRowData = this.memoizedGenerateTableRows(event, location);
    return (
            ...
            <Table
              ...
              getRowStyle={params => ({
                ...
                'background-color':
                  ...
                  location.selectedLocationSolutionSetId === params.data.locationSetId
                    ? location.selectedLocationSolutionId === params.data.locationSolutionId
  ...
  public componentDidUpdate(prevProps: LocationHistoryProps): void {
    const { event, setSelectedPreferredLocationSolution } = this.props;
    const latestLocationSS = getLatestLocationSolutionSetLegacy(event);
    if (prevProps.event.id !== event.id && latestLocationSS) {
      ...
      shouldUpdateSelectedLocationSolutionLegacy(prevProps.event, event) &&
      latestLocationSS
    ) {
      setSelectedPreferredLocationSolution(
        latestLocationSS.id,
  ...
  private readonly generateChildRows = (
    ...
    const childRows: LocationHistoryRow[] = flatMap(
      ...
      key => {
        const maybeLS: LegacyEventTypes.LocationSolution | undefined =
          locationSolutionSet.locationSolutions.find(
  ...
  private readonly generateChildRow = (
    ...
    const locationData: LocationHistoryRow = {
      ...
      latestLSSId: getLatestLocationSolutionSetLegacy(event).id,
  ...
  private readonly setToSave = (locationSolutionSetId: string) => {
    ...
    setSelectedPreferredLocationSolution(locationSolutionSetId, newPreferredId);
  };
  ...
  private readonly onRowClicked = (params: any): void => {
    const { setSelectedLocationSolution } = this.props;
    setSelectedLocationSolution(params.data.locationSetId, params.data.locationSolutionId);
  };
  ...
  private readonly onCellClicked = (params: any): void => {
    const { setSelectedPreferredLocationSolution } = this.props;
    if (params && params.column) {
      ...
      if (
        ...
        setSelectedPreferredLocationSolution(locationSetId, locationSolutionId);
      }
    }
  };
}
