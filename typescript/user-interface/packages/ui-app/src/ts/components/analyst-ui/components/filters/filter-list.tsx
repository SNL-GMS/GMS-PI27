...
import { setFocusToFiltersDisplay } from './filter-list-util';
 ...
 * @param setSelectedFilter setter for the filter
...
export const buildHandleKeyDown =
  (setActiveIndex, scrollIntoView, handleBlur, setSelectedFilter, filterList, activeIndex) => e => {
    if (e.key === 'Escape') {
      e.preventDefault();
      e.stopPropagation();
      (document.activeElement as HTMLElement).blur();
      handleBlur();
      setFocusToFiltersDisplay();
    } else if (e.key === 'Enter') {
      e.preventDefault();
      e.stopPropagation();
      const filter = filterList?.filters?.[activeIndex];
      if (filter) {
 ...
 */
export function FilterListComponent({ filterList }: FilterListProps) {
  ...
  const selectedFilterIndex = useSelectedFilterIndex();
...
export const FilterListOrNonIdealState = WithNonIdealStates<FilterListProps>(
  [
    {
      condition: props =>
        props.filterList == null || !props.filterList || props.filterList.length === 0,
