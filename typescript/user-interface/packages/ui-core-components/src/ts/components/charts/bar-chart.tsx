...
/**
 * builds the Victory Chart
 *
 * @param victoryChartKey victory chart key
 * @param victoryAxisKey victory chart axis key
 * @param GMSThemeBrushContainer gms them brush container
 * @param props props
 * @param scrollDomain scroll domain
 * @param setScrollDomain set scroll domain
 * @returns Victory Chart
 */
export const buildVictoryChart = (
  victoryChartKey: string,
  victoryAxisKey: string,
  GMSThemeBrushContainer: VictoryThemeDefinition,
  props: BarChartProps,
  scrollDomain: { min: number; max: number },
 ...
 */
export const buildScrollVictoryChart = (
  props: BarChartProps,
  scrollDomain: { min: number; max: number },
  setScrollDomain: React.Dispatch<
    React.SetStateAction<{
  ...
  if (GMSThemeBrushContainer?.independentAxis?.style?.tickLabels) {
    GMSThemeBrushContainer.independentAxis.style.tickLabels.fill = 'none';
  }

  if (GMSThemeBrushContainer?.independentAxis?.style?.ticks) {
  ...
  return buildVictoryChart(
    victoryChartKey,
    victoryAxisKey,
    GMSThemeBrushContainer,
    props,
    scrollDomain,
    setScrollDomain
  );
...
const buildVictoryBar = (props: BarChartProps, barWidth: number) => {
  ...
  return props.barDefs?.map(barDef => (
    <VictoryBar
      events={
                  ...
                  onContextMenu: e => [
                    {
                      ...
                      mutation: passedProps => {
                        if (props.onContextMenuBar) props.onContextMenuBar(e, passedProps);
                      }
                    }
                  ]
                }
...
export function BarChart(props: Readonly<BarChartProps>) {
  const width =
    props.widthPx - (props.padding?.right || 0) - (props.padding?.left || 0) - yAxisLabelsOffset;
