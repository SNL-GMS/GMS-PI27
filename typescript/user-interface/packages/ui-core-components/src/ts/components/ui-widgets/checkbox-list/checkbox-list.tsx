...
export class CheckboxList extends React.Component<CheckboxListProps, CheckboxListState> {
  ...
  private readonly buildEnumToColorMapDiv = (key: string) => {
    const classNameBox = this.shouldRenderNoColorSwatch(key) ? ' null-color-swatch' : '';
    return this.props.enumToColorMap ? (
      <div
        className={`checkbox-list__legend-box${classNameBox}`}
        style={{
  ...
  private readonly buildCheckBoxListLabel = (key: string) => {
    return this.props.enumKeysToDisplayStrings
          ...
          <React.Fragment key={`${key}rf`}>
            ...
            <div className="checkbox-list__row" key={key}>
              <div className="checkbox-list__box-and-label">
                ...
                >
                  <div className="checkbox-list__label">{this.buildCheckBoxListLabel(key)}</div>
                  {this.buildEnumToColorMapDiv(key)}
                </Checkbox>
              </div>
            </div>
  ...
  private readonly shouldRenderDivider = (key: string): boolean => {
    if (this.props.enumKeysToDividerMap) {
      return this.props.enumKeysToDividerMap.get(key) || false;
    }
    return false;
  };
  ...
  private readonly checkboxLabelString = (key: string): boolean | string => {
    if (this.props.enumKeysToLabelMap) {
      return this.props.enumKeysToLabelMap.get(key) || false;
    }
    return false;
  };
}
