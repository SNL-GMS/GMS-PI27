        ...
        {(leftIndicesToOverflow.length > 0 || rightIndicesToOverflow.length > 0) && (
          <Popover
            ...
            content={
              <Menu>
                {leftIndicesToOverflow.length > 0 &&
                  itemsLeft?.slice(itemsLeft.length - leftIndicesToOverflow.length).map(i => {
                    if (i != null && i.key !== null) {
                      return renderOverflowMenuItem(i, i.key);
                    }
                    return null;
                  })}
                {rightIndicesToOverflow.length > 0 &&
                  itemsRight?.slice(itemsRight.length - rightIndicesToOverflow.length).map(i => {
                    if (i != null && i.key !== null) {
                      return renderOverflowMenuItem(i, i.key);
                    }
                    return null;
