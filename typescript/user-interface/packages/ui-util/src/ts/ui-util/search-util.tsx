...
export function useSearch<T>(
  searchFn: (searchTerm: string) => SearchResult<T>[] | undefined
): [
...
export function useSearchResultSelectionManager<T>(
  ...
  const getSelectedResult = React.useCallback(
    () => results[selectedIndex],
    [results, selectedIndex]
  );
  const resetSelection = React.useCallback(() => setSelectedIndex(0), [setSelectedIndex]);
...
export const highlightRanges = (
  ...
  const suffix = text.substring(range ? range[1] + 1 : 0);
  return (
    <>
      {highlightRanges(prefix, mutableRanges)}
      <span className={highlightedClass}>
        {text.substring(range?.[0] || 0, range ? range[1] + 1 : 0)}
      </span>
      {suffix}
    </>
  );
