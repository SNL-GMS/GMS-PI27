...
export function hexToRgb(hex: ColorTypes.Hex): ColorTypes.RGB {
  ...
  if (result == null) {
    throw new Error(`Encountered an invalid hex value, ${hex}`);
  }
...
export function rgbToHex(rgb: ColorTypes.RGB): ColorTypes.Hex {
  ...
  return `#${((1 << 24) + (red << 16) + (green << 8) + blue).toString(BASE_SIXTEEN).slice(1)}`;
}
...
/**
 * set RGB and M values
 *
 * @param color the HSL color to convert
 * @returns the r,g,b and m values in a number array
 */
export const setHexRGB = (color: HSL): number[] => {
...
export const hslToHex = (color: HSL): ColorTypes.Hex => {
  const rgbmValues = setHexRGB(color);
  // Having obtained RGB, convert channels to hex
  let rStr = Math.round((rgbmValues[0] + rgbmValues[3]) * 255).toString(BASE_SIXTEEN);
  let gStr = Math.round((rgbmValues[1] + rgbmValues[3]) * 255).toString(BASE_SIXTEEN);
  let bStr = Math.round((rgbmValues[2] + rgbmValues[3]) * 255).toString(BASE_SIXTEEN);
...
export const isColorLegible = (
  ...
  return contrast(convertToRGB(background), convertToRGB(foreground)) > MINIMUM_CONTRAST;
};
...
export class DistinctColorPalette {
   ...
   */
  public addColor(): HSL {
    const hsl = this.generateDistinctColorHSL();
    this.colorMap = this.colorMap.set(this.colorMap.size, hsl);
    return hsl;
  }
  ...
  public getColor(key: number | string): HSL {
    ...
    const hsl = this.colorMap.get(key);
  ...
  private readonly generateDistinctColorHSL = (): HSL => {
    ...
    return hsvToHSL(this.nextHue, DEFAULT_SATURATION, DEFAULT_VALUE);
  };
  ...
  private readonly getHueFromSeed = (seed: number | string | undefined): number => {
    if (!seed) {
      return 0;
    }
...
export const decimalToHex = (decimal: number): string => {
  return (decimal * 255).toString(BASE_SIXTEEN);
};
...
export function blendColors(
  ...
  const matchColorA = colorA.match(/\w\w/g);
  const matchColorB = colorB.match(/\w\w/g);

  if (!matchColorA) {
    throw new Error(`Unable to match color A`);
  }

  if (!matchColorB) {
    throw new Error(`Unable to match color B`);
  }

  const [rA, gA, bA] = matchColorA.map(c => parseInt(c, 16));
  const [rB, gB, bB] = matchColorB.map(c => parseInt(c, 16));
...
/**
 * Blends a color with black to produce a darker color
 *
 * @param color a hex color string
 * @param percentage 0 to 1
 * @returns a color blended with black to the percentage provided. 1 is all black. 0 is all the original color.
 */
export function darkenColor(color: string, percentage: number) {
  return blendColors(color, '#000000', percentage);
}

/**
 * Blends a color with white to produce a lighter color
 *
 * @param color a hex color string
 * @param percentage 0 to 1
 * @returns a color blended with white to the percentage provided. 1 is all black. 0 is all the original color.
 */
export function lightenColor(color: string, percentage: number) {
  return blendColors(color, '#ffffff', percentage);
}

/**
 * ! Jet is terrible, but some users still insist on using it.
 * It should never be set as a default, but is included for the sake of completeness.
 *
 * See @link [No More Rainbows](https://agilescientific.com/blog/2017/12/14/no-more-rainbows)
 */
export const interpolateJet = (t: number) => {
  /* eslint-disable @typescript-eslint/no-magic-numbers */
  if (t < 0 || t > 1) {
