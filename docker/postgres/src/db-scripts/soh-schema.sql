...
(
	...
	authentication_status integer not null,
...
create sequence if not exists channel_env_issue_analog_sequence increment by 1 minvalue 1 no maxvalue start with 1 no cycle;
create sequence if not exists channel_env_issue_boolean_sequence increment by 1 minvalue 1 no maxvalue start with 1 no cycle;
...
(
	...
	user_profile_id varchar(255) not null
		constraint fk_user_profile_workspace_layout
...
create table if not exists soh_status_change_collection(
  ...
  soh_monitor_type integer not null,
...
create table if not exists capability_soh_rollup(
  ...
  group_rollup_status integer not null,
  station_group_name varchar(255) not null
);
...
create table if not exists system_message(
    ...
    system_message_severity integer not null,
    system_message_category integer not null,
    system_message_sub_category integer not null,
    messageTags jsonb
);
...
grant usage on sequence channel_env_issue_analog_sequence to gms_admin;
grant usage on sequence channel_env_issue_boolean_sequence to gms_admin;
...
grant usage on sequence channel_env_issue_analog_sequence to gms_soh_application;
grant usage on sequence channel_env_issue_boolean_sequence to gms_soh_application;
...
grant delete on channel_group to gms_soh_application;
grant delete on station_group_stations to gms_soh_application;
grant delete on channel_group_channels to gms_soh_application;
grant delete on station_channel_info to gms_soh_application;
...
grant usage on sequence channel_env_issue_analog_sequence to gms_soh_application_elevated;
grant usage on sequence channel_env_issue_boolean_sequence to gms_soh_application_elevated;
...
grant delete on channel_group to gms_soh_application_elevated;
grant delete on station_group_stations to gms_soh_application_elevated;
grant delete on channel_group_channels to gms_soh_application_elevated;
grant delete on station_channel_info to gms_soh_application_elevated;
...
alter sequence channel_env_issue_analog_sequence owner to gms_admin;
alter sequence channel_env_issue_boolean_sequence owner to gms_admin;
...
alter table user_profile owner to gms_admin;
