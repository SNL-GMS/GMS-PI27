...
class KubeCtl:
    ...
    def validate_namespace(self) -> None:
        ...
        if not pattern.match(self.namespace):
            raise ValueError(
                ...
                f"{self.namespace!r} is invalid."
            )

    def all_pods_ready(self) -> bool:
        """
        Get all the pods in the namespace, and determine whether they're
        all ready to go (their "STATUS" is either "Completed", or it's
        "Running" and all the containers are "READY").

        Returns:
            Whether all the pods in the given namespace are ready.
        """
        pod_table = self.get_pods(header=False)
        if not pod_table:
            return False
    ...
    def _all_pods_ready_retry_wrapper(self) -> None:
        """
        Wrap :func:`all_pods_ready` to make it suitable for retrying.

        Raises:
            RuntimeError:  If a pod "Failed" or returned an unknown
                status.
            TryAgain:  If not all the pods are "Running" or "Completed"
                yet.
        """
        if not self.all_pods_ready():
            raise TryAgain
        ...
        **extra_retrying_kwargs
        ...
        Args:
            ...
            max_attempts:  If specified, the maximum number of times to
                try calling :func:`_all_pods_ready_retry_wrapper`.
            extra_retrying_kwargs:  Any additional keyword arguments to
                pass on to the :class:`Retrying` constructor.

        Raises:
            RetryError:  If the pods aren't all "Running" or "Completed"
        ...
        retry(self._all_pods_ready_retry_wrapper)

    def get_pods(self, header: bool = True) -> str:
        """
        Get the results of ``kubectl get pods``.

        Args:
            header:  Whether to include the header row in the output.

        Returns:
            The plain-text table of pods in the namespace.
        """
        command = f"kubectl get pods --namespace {self.namespace}"
        if not header:
            command += " --no-headers"
    ...
    def get_logs_from_container(
        ...
        Args:
            ...
            previous:  Whether to use the ``--previous`` flag to get the
                logs from the previous container.

        Returns:
            The logs from the container.
        """
        cmd = f"kubectl --namespace {self.namespace} logs {pod} {container}"
        if previous:
            cmd += " --previous"
        return subprocess.run(
            cmd,
    ...
    RESOURCE_SELECTORS = {
        "deployment": "app.kubernetes.io/name",
        "statefulset": "app.kubernetes.io/name",
        "job": "app.kubernetes.io/name"
    }
    """
    ...
    def get_resources_for_namespace(self, resource_type: str) -> list[str]:
        ...
        return subprocess.run(
            ...
            r"--output custom-columns=NAME:'.metadata.labels.app\.kubernetes\.io/name' --no-headers",
            shell=True,
            capture_output=True,
            text=True
        ).stdout.strip().splitlines()
    ...
    def save_resource_logs(
        ...
        for pod in pods:
            ...
            for container in containers:
                ...
                previous_log = self.get_logs_from_container(
                    pod,
                    container,
                    previous=True
                )
                ...
                if previous_log:
                    with open(
                        log_dir / f"{pod}.{container}.previous.log.txt",
                        "w"
                    ) as f:
    ...
    def get_resource(
        ...
        """
        selector_argument = "".join(
            f"--selector={s} " for s in selectors or []
        )
        ...
        if completed_process.returncode != 0:
            ...
            return results["items"] if "items" in results else [results]
