...
from . import app_state, executor, log_queue
...
def config_services_available() -> bool:
    """
    Return True if the processing configuration and
    osd services are both available.
    """
    processing_config_service_ready = False
    user_manager_service_ready = False
    minio_service_ready_or_nonexistent = False

    try:
        response = requests.get(
            f'http://{current_app.config["PROCESSING_CONFIG_SERVICE_NAME"]}:'
            f'8080/{current_app.config["PROCESSING_CONFIG_SERVICE_NAME"]}/'
            "alive",
            timeout=1
        )
        if response.status_code == 200:
            ...
            logger.info(
                f'{current_app.config["PROCESSING_CONFIG_SERVICE_NAME"]} '
                "not alive... waiting"
            )
    ...
    try:
        response = requests.get(
            f'http://{current_app.config["USER_MANAGER_SERVICE_NAME"]}:8080/'
            f'{current_app.config["USER_MANAGER_SERVICE_NAME"]}/alive',
            timeout=1
        )
        if response.status_code == 200:
            user_manager_service_ready = True
        else:
            logger.info(
                f'{current_app.config["USER_MANAGER_SERVICE_NAME"]} '
                "not alive... waiting"
            )
    ...
    # If the minio credentials are not set in the environment,
    # assume we don't need to load minio.
    if os.getenv('MINIO_ROOT_USER') is not None:
        # TODO: Look into how to handle both IAN and SOH.
        try:
            response = requests.get(
                f'http://{current_app.config["MINIO_SERVICE_NAME"]}:9000/'
                "minio/health/ready"
            )
            if response.status_code == 200:
                ...
                logger.info(
                    f'{current_app.config["MINIO_SERVICE_NAME"]} '
                    "not ready... waiting"
                )
    ...
    return (
        processing_config_service_ready
        and minio_service_ready_or_nonexistent and user_manager_service_ready
    )


def dataload(
    load_processing_config: bool = False,
    ...
    try:
        processing_config_url = (
            f'http://'
            f'{current_app.config["PROCESSING_CONFIG_SERVICE_NAME"]}:8080/'
            f'{current_app.config["PROCESSING_CONFIG_SERVICE_NAME"]}'
        )
        ...
        dataloader = GmsDataloader(
            current_app.config["BASE_CONFIG_PATH"],
        ...
        if load_earth_models and os.getenv("MINIO_ROOT_USER") is not None:
            ...
            dataloader.load_earthmodel_data(
                minio_url,
                current_app.config["MINIO_FEATURE_PREDICTION_SERVICE_BUCKET"]
            )
            logger.info("Earth models loaded successfully")

        if load_dispersion and os.getenv("MINIO_ROOT_USER") is not None:
            minio_url = f'{ current_app.config["MINIO_SERVICE_NAME"] }:9000'
            logger.info("Loading dispersion...")
            dataloader.load_dispersion_data(
                minio_url,
                current_app.config["MINIO_DISPERSION_BUCKET"]
            )
            logger.info("Dispersion loaded successfully")

        if load_mediumvelocity and os.getenv("MINIO_ROOT_USER") is not None:
            minio_url = f'{current_app.config["MINIO_SERVICE_NAME"]}:9000'
            logger.info("Loading mediumvelocity...")
            dataloader.load_mediumvelocity_data(
                minio_url,
                current_app.config["MINIO_FEATURE_PREDICTION_SERVICE_BUCKET"]
            )
        ...
        if load_dg and os.getenv("MINIO_ROOT_USER") is not None:
            ...
            dataloader.load_dziewonski_gilbert_data(
                minio_url,
                current_app.config["MINIO_FEATURE_PREDICTION_SERVICE_BUCKET"]
            )
        ...
        # logger.error(ex)
        logger.error("There has been an exception")
        return False

    delta_time = datetime.now() - start_time
    logger.info(
        f'Dataload completed successfully in { str(delta_time).split(".")[0] }'
    )  # leave off microseconds
    return True
...
def loader(reload: bool = False) -> tuple[bool, str]:
    ...
    while True:
        ...
        if config_services_available():
            ...
            # add a logging handler to capture dataload logging
            # into a string
            ...
            if not reload:
                load_successful = dataload(
                    load_processing_config=True,
                    load_user_preferences=True,
                    load_earth_models=True,
                    load_dispersion=True,
            ...
            current_app.config['SERVICE_WAIT_TIMEOUT']
        ):
            ...
            loader_log_output = (
                f'{ action } TIMED OUT: config services still '
                f'unavailable after { waited } seconds'
            )
